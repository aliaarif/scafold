<!DOCTYPE html>
<html lang="en">

<head>
    <!-- Required Meta Tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta http-equiv="x-ua-compatible" content="ie=edge">

    <title> Bulkit :: Lists</title>
    <link rel="icon" type="image/png" href="assets/img/favicon.png" />

    <!--Core CSS -->
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="assets/css/bulma.css">
    <link rel="stylesheet" href="assets/css/app.css">
    <link rel="stylesheet" href="assets/css/core.css">
</head>

<body id="component-page">
    <div class="pageloader"></div>
    <div class="infraloader is-active"></div>
    <!-- Hero and nav -->
    <div class="hero is-relative is-theme-primary">

        <nav class="navbar navbar-wrapper navbar-light is-transparent is-static">
            <div class="container">
                <!-- Brand -->
                <div class="navbar-brand">
                    <a class="navbar-item" href="index.php">
                        <img src="assets/img/logos/bulkit-white.svg" alt="">
                    </a>

                    <!-- Sidebar Trigger -->
                    <a id="navigation-trigger" class="navbar-item hamburger-btn" href="javascript:void(0);">
                        <span class="menu-toggle">
                                <span class="icon-box-toggle">
                                    <span class="rotate">
                                        <i class="icon-line-top"></i>
                                        <i class="icon-line-center"></i>
                                        <i class="icon-line-bottom"></i>
                                    </span>
                        </span>
                        </span>
                    </a>

                    <!-- Responsive toggle -->
                    <div class="custom-burger" data-target="">
                        <a id="" class="responsive-btn" href="javascript:void(0);">
                            <span class="menu-toggle">
                                    <span class="icon-box-toggle">
                                        <span class="rotate">
                                            <i class="icon-line-top"></i>
                                            <i class="icon-line-center"></i>
                                            <i class="icon-line-bottom"></i>
                                        </span>
                            </span>
                            </span>
                        </a>
                    </div>
                    <!-- /Responsive toggle -->
                </div>

                <!-- Navbar menu -->
                <div class="navbar-menu">
                    <!-- Navbar Start -->
                    <div class="navbar-start">
                        <!-- Dropdown -->
                        <div class="navbar-item has-dropdown is-hoverable is-mega">
                            <div class="navbar-link">
                                Menu
                            </div>
                            <div class="navbar-dropdown is-megamenu animated prefadeInUp fadeInUp" data-style="width: 18rem;">
                                <div class="mega-inner">
                                    <div class="container">
                                        <div class="columns">
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Landing Kits</h2>
                                                <div class="scrollable-container" data-simplebar>
                                                    <a class="navbar-item is-flex" href="kit1-landing.php">
                                                        <i class="im im-icon-Navigation-RightWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 1</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit2-landing.php">
                                                        <i class="im im-icon-Split-Horizontal"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 2</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit3-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 3</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit4-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 4</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit5-landing.php">
                                                        <i class="im im-icon-Love-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 5</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit6-landing.php">
                                                        <i class="im im-icon-Two-Windows"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 6</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit7-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 7</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit5-landing.php">
                                                        <i class="im im-icon-Love-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 5</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit6-landing.php">
                                                        <i class="im im-icon-Two-Windows"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 6</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit7-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 7</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit8-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 8</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit9-landing.php">
                                                        <i class="im im-icon-Split-FourSquareWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 9</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit10-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 10</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit11-landing.php">
                                                        <i class="im im-icon-Favorite-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 11</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit12-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 12</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit13-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 13</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit14-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 14</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit15-landing.php">
                                                        <i class="im im-icon-Split-FourSquareWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 15</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Utility Pages</h2>
                                                <div class="scrollable-container" data-simplebar="">
                                                    <a class="navbar-item is-flex" href="about-page-1.php">
                                                        <i class="im im-icon-Building"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">ABOUT</small>
                                                            </p>
                                                            <p>About v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="about-page-2.php">
                                                        <i class="im im-icon-Building"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">ABOUT</small>
                                                            </p>
                                                            <p>About v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="about-page-2.php">
                                                        <i class="im im-icon-Structure"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CASE STUDY</small>
                                                            </p>
                                                            <p>Case Study v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-1.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-2.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-3.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-4.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-5.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v5</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-full-alt.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-side-alt.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-full-masonry.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-side-masonry.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-single-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Single Post v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-single-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Single Post v2</p>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Themed Kits</h2>
                                                <a class="navbar-item is-flex" href="agency-home.php">
                                                    <i class="im im-icon-Post-Office"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">AGENCY</small>
                                                        </p>
                                                        <p>Agency landing</p>
                                                    </div>
                                                </a>
                                                <a class="navbar-item is-flex" href="startup-home.php">
                                                    <i class="im im-icon-Factory"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">STARTUP</small>
                                                        </p>
                                                        <p>Startup landing</p>
                                                    </div>
                                                </a>
                                                <a class="navbar-item is-flex" href="error-1.php">
                                                    <i class="im im-icon-Coffin"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">ERROR KIT</small>
                                                        </p>
                                                        <p>Error Pages</p>
                                                    </div>
                                                </a>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">More goodies</h2>
                                                <p class="is-cta">As a Bulkit customer, you get some free goodies on Css Ninja.</p>
                                                <p class="is-cta">
                                                    <a href="https://cssninja.io" target="_blank" class="button btn-align mega-btn primary-btn">Learn more</a>
                                                </p>
                                            </div>
                                        </div>
                                    </div>

                                    <hr class="navbar-divider">
                                    <div class="navbar-item is-footer is-hidden-mobile">
                                        <div class="navbar-content">
                                            <div class="level is-mobile">
                                                <div class="level-left">
                                                    <div class="level-item">
                                                        <strong>Follow us</strong>
                                                    </div>
                                                </div>
                                                <div class="level-right">
                                                    <div class="level-item">
                                                        <a class="social-link" href="_components-basicui-lists.php#"><i class="fa fa-twitter"></i></a>
                                                        <a class="social-link" href="_components-basicui-lists.php#"><i class="fa fa-facebook"></i></a>
                                                        <a class="social-link" href="_components-basicui-lists.php#"><i class="fa fa-behance"></i></a>
                                                        <a class="social-link" href="_components-basicui-lists.php#"><i class="fa fa-github"></i></a>
                                                        <a class="social-link" href="_components-basicui-lists.php#"><i class="fa fa-tumblr"></i></a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="menu-image is-hidden-mobile">
                                        <img src="assets/img/graphics/legacy/megamenu-core.svg" data-base-url="assets/img/graphics/legacy/megamenu" data-extension=".svg" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Mobile dropdown -->
                        <!--div class="mobile-drop is-hidden-desktop">
                                <div class="mobile-drop-trigger">
                                    V1 <i class="material-icons">keyboard_arrow_right</i>
                                </div>
                                <div class="child-menu">
                                    <ul>
                                        <li>
                                            <a href="/agency-home.php">Agency kit</a>
                                        </li>
                                        <li>
                                            <a href="/startup-landing.php">Startup kit</a>
                                        </li>
                                        <li>
                                            <a href="/kit1-landing.php">Landing kit 1</a>
                                        </li>
                                        <li>
                                            <a href="/kit2-landing.php">Landing kit 2</a>
                                        </li>
                                        <li>
                                            <a href="/kit3-landing.php">Landing kit 3</a>
                                        </li>
                                        <li>
                                            <a href="/kit4-landing.php">Landing kit 4</a>
                                        </li>
                                        <li>
                                            <a href="/kit5-landing.php">Landing kit 5</a>
                                        </li>
                                        <li>
                                            <a href="/kit6-landing.php">Landing kit 6</a>
                                        </li>
                                        <li>
                                            <a href="/kit7-landing.php">Landing kit 7</a>
                                        </li>
                                        <li>
                                            <a href="/kit8-landing.php">Landing kit 8</a>
                                        </li>
                                        <li>
                                            <a href="/kit9-landing.php">Landing kit 9</a>
                                        </li>
                                        <li>
                                            <a href="/kit10-landing.php">Landing kit 10</a>
                                        </li>
                                        <li>
                                            <a href="/kit11-landing.php">Landing kit 11</a>
                                        </li>
                                    </ul>
                                </div>                </div-->
                        <!-- Navbar item -->
                        <a class="navbar-item is-slide" href="https://docs.bulkit.cssninja.io/" target="_blank">
                            Docs
                        </a>
                        <!-- Navbar item -->
                        <a class="navbar-item is-slide" href="https://cssninja.io/" target="_blank">
                            Help
                        </a>
                    </div>

                    <!-- Navbar end -->
                    <div class="navbar-end">
                        <!-- Signup button -->
                        <div class="navbar-item">
                            <a id="#signup-btn" href="https://1.envato.market/c/1288816/275988/4415?subId1=button&subId2=banner&subId3=https%3A%2F%2Fthemeforest.net%2Fitem%2Fbulkit-agency-startup-and-saas-template%2F21278677&u=https%3A%2F%2Fthemeforest.net%2Fitem%2Fbulkit-agency-startup-and-saas-template%2F21278677" class="button button-cta btn-outlined is-bold btn-align light-btn rounded raised">
                                Buy now
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </nav>
        <!-- Hero image -->
        <div id="main-hero" class="hero-body">
            <div class="container has-text-centered">
                <div class="columns is-vcentered">
                    <div class="column is-4 is-offset-1 has-text-left">
                        <h1 class="title components-title is-1">
                            Lists
                        </h1>
                        <h2 class="subtitle is-4 components-subtitle">
                            Available lists variations.
                        </h2>
                        <p class="components-cta">
                            <a href="_components-basicui-lists.php#common-lists" class="button button-cta btn-align light-btn btn-outlined is-bold rounded">
                                Get Started
                            </a>
                        </p>
                    </div>
                    <div class="column is-offset-1 has-text-centered is-hidden-mobile">
                        <div id="lists-ill" class="vivus-svg"></div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Scrollnav -->
    <div id="scrollnav" class="scroll-nav-wrapper">
        <div class="container">
            <div class="tabs scrollnav-tabs is-centered">
                <ul>
                    <li class="scrollnav-item is-active"><a href="_components-basicui-lists.php#common-lists">Common lists</a></li>
                    <li class="scrollnav-item"><a href="_components-basicui-lists.php#levels">Levels</a></li>
                    <li class="scrollnav-item"><a href="_components-basicui-lists.php#special-lists">Special lists</a></li>
                </ul>
            </div>
        </div>
    </div>

    <div class="section">
        <div class="container">
            <div id="common-lists" class="section-title-wrapper has-text-centered">
                <div class="bg-number">1</div>
                <h2 class="title section-title has-text-centered revealOnScroll delay-1 dark-text text-bold">
                    Common lists
                </h2>
            </div>

            <!-- Basic list -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Basic list</h4>
                        <p class="no-padding">To create a basic list, jus create a <code>ul</code> or a <code>ol</code> structure with several <code>li</code> elements nested inside. The result is a simple list, without bullets or numbers.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <ul>
                        <li>Item 1</li>
                        <li>Item 2</li>
                        <li>Item 3</li>
                        <li>Item 4</li>
                        <li>Item 5</li>
                    </ul>
                </div>
            </div>

            <!-- Bulleted list -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Content list</h4>
                        <p class="no-padding">If you dont want to create a standard bulleted list, just wrap the list structure we saw previsously with a <code>.content</code> <code>div</code> element. Bullets will then show. Numbers will be also be shown if it is an ordered list.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <div class="content">
                        <ul>
                            <li>Item 1</li>
                            <li>Item 2</li>
                            <li>Item 3</li>
                            <li>Item 4</li>
                            <li>Item 5</li>
                        </ul>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;content&#x22;&#x3E;
            &#x3C;ul&#x3E;
                &#x3C;li&#x3E;Item 1&#x3C;/li&#x3E;
                &#x3C;li&#x3E;Item 2&#x3C;/li&#x3E;
                &#x3C;li&#x3E;Item 3&#x3C;/li&#x3E;
                &#x3C;li&#x3E;Item 4&#x3C;/li&#x3E;
                &#x3C;li&#x3E;Item 5&#x3C;/li&#x3E;
            &#x3C;/ul&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Icon bullet list -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block pb-20">
                        <h4>Icon bullet</h4>
                        <p class="no-padding">You can customize the glyph or bullet being shown at the left of unordered list items. You can replace it with a font icon. Add the <code>.icon-bullet-list</code> class to an unordered list to activate it. Default icon is <code>fa fa-circle-o</code>. It could be anything else, you just have to make the changes in the related css snippet. </p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <ul class="icon-bullet-list">
                        <li>Item 1</li>
                        <li>Item 2</li>
                        <li>Item 3</li>
                        <li>Item 4</li>
                        <li>Item 5</li>
                    </ul>
                </div>
            </div>

            <div class="columns is-vcentered huge-pb">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;ul class=&#x22;icon-bullet-list&#x22;&#x3E;
            &#x3C;li&#x3E;Item 1&#x3C;/li&#x3E;
            &#x3C;li&#x3E;Item 2&#x3C;/li&#x3E;
            &#x3C;li&#x3E;Item 3&#x3C;/li&#x3E;
            &#x3C;li&#x3E;Item 4&#x3C;/li&#x3E;
            &#x3C;li&#x3E;Item 5&#x3C;/li&#x3E;
        &#x3C;/ul&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Solid list -->
            <div class="columns is-vcentered mt-80">
                <div class="column is-6 is-offset-3">
                    <div class="info-block pb-20">
                        <h4>Solid list</h4>
                        <p class="no-padding">You can build customized lists with solid lists. However, it doesnt rely on <code>ul</code> or <code>ol</code> for the structure, but rather on <code>div</code>. You can have anything you want as a bullet, including icons and images. Check the code sample for more details.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <div class="solid-list">
                        <div class="solid-list-item">
                            <div class="list-bullet">
                                <i class="sl sl-icon-check"></i>
                            </div>
                            <div class="list-text">
                                This is a list item text
                            </div>
                        </div>
                        <div class="solid-list-item">
                            <div class="list-bullet">
                                <i class="sl sl-icon-check"></i>
                            </div>
                            <div class="list-text">
                                This is another list item text
                            </div>
                        </div>
                        <div class="solid-list-item">
                            <div class="list-bullet">
                                <i class="sl sl-icon-check"></i>
                            </div>
                            <div class="list-text">
                                This is a third list item text
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;solid-list&#x22;&#x3E;
            &#x3C;div class=&#x22;solid-list-item&#x22;&#x3E;
                &#x3C;div class=&#x22;list-bullet&#x22;&#x3E;
                    &#x3C;i class=&#x22;sl sl-icon-check&#x22;&#x3E;&#x3C;/i&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;list-text&#x22;&#x3E;
                    This is a list item text
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;solid-list-item&#x22;&#x3E;
                &#x3C;div class=&#x22;list-bullet&#x22;&#x3E;
                    &#x3C;i class=&#x22;sl sl-icon-check&#x22;&#x3E;&#x3C;/i&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;list-text&#x22;&#x3E;
                    This is another list item text
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;solid-list-item&#x22;&#x3E;
                &#x3C;div class=&#x22;list-bullet&#x22;&#x3E;
                    &#x3C;i class=&#x22;sl sl-icon-check&#x22;&#x3E;&#x3C;/i&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;list-text&#x22;&#x3E;
                    This is a third list item text
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Inline list -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Inline list</h4>
                        <p class="no-padding">You can make a list display horizontally. Just add the <code>.inline-list</code> class to the <code>ul</code> element. List separators are made of font awesome icons. You are free to change them with an appropriate element.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <div class="inline-list has-text-centered">
                        <div>Item 1</div>
                        <i class="inline-bullet fa fa-circle"></i>
                        <div>Item 2</div>
                        <i class="inline-bullet fa fa-circle"></i>
                        <div>Item 3</div>
                        <i class="inline-bullet fa fa-circle"></i>
                        <div>Item 4</div>
                        <i class="inline-bullet fa fa-circle"></i>
                        <div>Item 5</div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;inline-list&#x22;&#x3E;
            &#x3C;div&#x3E;Item 1&#x3C;/div&#x3E;
            &#x3C;i class=&#x22;inline-bullet fa fa-circle&#x22;&#x3E;&#x3C;/i&#x3E;
            &#x3C;div&#x3E;Item 2&#x3C;/div&#x3E;
            &#x3C;i class=&#x22;inline-bullet fa fa-circle&#x22;&#x3E;&#x3C;/i&#x3E;
            &#x3C;div&#x3E;Item 3&#x3C;/div&#x3E;
            &#x3C;i class=&#x22;inline-bullet fa fa-circle&#x22;&#x3E;&#x3C;/i&#x3E;
            &#x3C;div&#x3E;Item 4&#x3C;/div&#x3E;
            &#x3C;i class=&#x22;inline-bullet fa fa-circle&#x22;&#x3E;&#x3C;/i&#x3E;
            &#x3C;div&#x3E;Item 5&#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- List block minimal -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>List block minimal</h4>
                        <p class="no-padding">List blocks are very handy and act as interactive lists, such as menus or options. They can also have badges appended to list items. Here is the structure of a minimal list block. Active list items are by default highlighted with the <code>$primary</code> color.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <ul class="list-block minimal">
                        <li class="is-active"><a>This is a list block item</a> <span class="b-badge is-primary">9</span></li>
                        <li><a>It is very handy for menus</a></li>
                        <li><a>Use it as a sticky element</a></li>
                        <li><a>It's cool</a> <span class="b-badge is-primary">5</span></li>
                        <li><a>Support team is great</a></li>
                    </ul>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;ul class=&#x22;list-block minimal&#x22;&#x3E;
            &#x3C;li class=&#x22;is-active&#x22;&#x3E;&#x3C;a&#x3E;This is a list block item&#x3C;/a&#x3E; &#x3C;span class=&#x22;b-badge is-primary&#x22;&#x3E;9&#x3C;/span&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;It is very handy for menus&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;Use it as a sticky element&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;It&#x27;s cool&#x3C;/a&#x3E; &#x3C;span class=&#x22;b-badge is-primary&#x22;&#x3E;5&#x3C;/span&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;Support team is great&#x3C;/a&#x3E;&#x3C;/li&#x3E;
        &#x3C;/ul&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- List block -->
            <div class="columns is-vcentered pt60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>List block</h4>
                        <p class="no-padding">A list block can appear like a small panel with a light shadow. This is the list block default behaviour.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <ul class="list-block">
                        <li class="is-active"><a>This is a list block item</a> <span class="b-badge rounded is-primary">9</span></li>
                        <li><a>It is very handy for menus</a></li>
                        <li><a>Use it as a sticky element</a></li>
                        <li><a>It's cool</a> <span class="b-badge rounded is-primary">5</span></li>
                        <li><a>Support team is great</a></li>
                    </ul>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;ul class=&#x22;list-block&#x22;&#x3E;
            &#x3C;li class=&#x22;is-active&#x22;&#x3E;&#x3C;a&#x3E;This is a list block item&#x3C;/a&#x3E; &#x3C;span class=&#x22;b-badge rounded is-primary&#x22;&#x3E;9&#x3C;/span&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;It is very handy for menus&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;Use it as a sticky element&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;It&#x27;s cool&#x3C;/a&#x3E; &#x3C;span class=&#x22;b-badge rounded is-primary&#x22;&#x3E;5&#x3C;/span&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;Support team is great&#x3C;/a&#x3E;&#x3C;/li&#x3E;
        &#x3C;/ul&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- List block with icons -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>List block with icons</h4>
                        <p class="no-padding">You can add icons to a list block element items. Insert the icon before the item's text. It can be from any icon family.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <ul class="list-block">
                        <li class="is-active"><a><i class="sl sl-icon-star mr-20"></i> This is a list block item</a></li>
                        <li><a><i class="sl sl-icon-trophy mr-20"></i> It is very handy for menus</a></li>
                        <li><a><i class="sl sl-icon-cup mr-20"></i> Use it as a sticky element</a></li>
                        <li><a><i class="sl sl-icon-badge mr-20"></i> It's cool</a></li>
                        <li><a><i class="sl sl-icon-support mr-20"></i> Support team is great</a></li>
                    </ul>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;ul class=&#x22;list-block&#x22;&#x3E;
            &#x3C;li class=&#x22;is-active&#x22;&#x3E;&#x3C;a&#x3E;&#x3C;i class=&#x22;sl sl-icon-star mr-20&#x22;&#x3E;&#x3C;/i&#x3E; This is a list block item&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;&#x3C;i class=&#x22;sl sl-icon-trophy mr-20&#x22;&#x3E;&#x3C;/i&#x3E; It is very handy for menus&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;&#x3C;i class=&#x22;sl sl-icon-cup mr-20&#x22;&#x3E;&#x3C;/i&#x3E; Use it as a sticky element&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;&#x3C;i class=&#x22;sl sl-icon-badge mr-20&#x22;&#x3E;&#x3C;/i&#x3E; It&#x27;s cool&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;&#x3C;i class=&#x22;sl sl-icon-support mr-20&#x22;&#x3E;&#x3C;/i&#x3E; Support team is great&#x3C;/a&#x3E;&#x3C;/li&#x3E;
        &#x3C;/ul&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- List block bordered -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>List block bordered</h4>
                        <p class="no-padding">Add the <code>.bordered</code> class to a list block to make its items bordered.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <ul class="list-block bordered">
                        <li class="is-active"><a>This is a list block item</a></li>
                        <li><a>It is very handy for menus</a></li>
                        <li><a>Use it as a sticky element</a></li>
                        <li><a>It's cool</a></li>
                        <li><a>Support team is great</a></li>
                    </ul>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;ul class=&#x22;list-block bordered&#x22;&#x3E;
            &#x3C;li class=&#x22;is-active&#x22;&#x3E;&#x3C;a&#x3E;This is a list block item&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;It is very handy for menus&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;Use it as a sticky element&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;It&#x27;s cool&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;li&#x3E;&#x3C;a&#x3E;Support team is great&#x3C;/a&#x3E;&#x3C;/li&#x3E;
        &#x3C;/ul&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Active item colors -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Active item colors</h4>
                        <p class="no-padding">You can change the background color of active items. In order to do this, add the <code>.is-secondary</code> or the <code>.is -accent</code> class to the <code>ul</code> element. The list block active background color defaults to <code>$primary</code>.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered pb-60">
                <div class="column is-4">
                    <ul class="list-block">
                        <li class="is-active"><a>This is a Primary active item</a></li>
                        <li><a>It is very handy for menus</a></li>
                        <li><a>Use it as a sticky element</a></li>
                        <li><a>It's cool</a></li>
                        <li><a>Support team is great</a></li>
                    </ul>
                </div>
                <div class="column is-4">
                    <ul class="list-block is-secondary">
                        <li class="is-active"><a>This is a Secondary active item</a></li>
                        <li><a>It is very handy for menus</a></li>
                        <li><a>Use it as a sticky element</a></li>
                        <li><a>It's cool</a></li>
                        <li><a>Support team is great</a></li>
                    </ul>
                </div>
                <div class="column is-4">
                    <ul class="list-block is-accent">
                        <li class="is-active"><a>This is an Accent active item</a></li>
                        <li><a>It is very handy for menus</a></li>
                        <li><a>Use it as a sticky element</a></li>
                        <li><a>It's cool</a></li>
                        <li><a>Support team is great</a></li>
                    </ul>
                </div>
            </div>

        </div>
    </div>

    <div class="section section-feature-grey">
        <div class="container">
            <div id="levels" class="section-title-wrapper has-text-centered pt-60">
                <div class="bg-number">2</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Levels
                </h2>
            </div>

            <!-- Basic level -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <span class="tag is-secondary">Reworked 3.0.0</span>
                        <h4>Basic level</h4>
                        <p class="no-padding">Level is a versatile component brought by Bulma. It basically imitates the behaviour of an inline list. However, you can insert almost any element inside a level. The default level has two parts : <code>.level-left</code> and <code>.level-right</code>. Each one of these parts contains level items. By default, level items are displayed vertically on mobile. Check the code sample for more details about level structure.</p>
                    </div>
                </div>
            </div>

            <div class="columns">
                <div class="column is-8 is-offset-2">
                    <nav class="level">
                        <!-- Left side -->
                        <div class="level-left">
                            <div class="level-item">
                                <p class="subtitle is-5">
                                    <strong>123</strong> tasks
                                </p>
                            </div>
                            <div class="level-item">
                                <div class="field has-addons">
                                    <p class="control no-margin-bottom">
                                        <input class="input" type="text" placeholder="Find a task">
                                    </p>
                                    <p class="control">
                                        <button class="button btn-align accent-btn raised">
                                            Search
                                        </button>
                                    </p>
                                </div>
                            </div>
                        </div>

                        <!-- Right side -->
                        <div class="level-right">
                            <p class="level-item"><i class="sl sl-icon-grid is-icon-md"></i></p>
                            <p class="level-item"><i class="sl sl-icon-user is-icon-md"></i></p>
                            <p class="level-item"><i class="sl sl-icon-picture is-icon-md"></i></p>
                            <p class="level-item"><i class="sl sl-icon-check is-icon-md"></i></p>
                            <p class="level-item"><a class="button btn-align accent-btn raised">New task</a></p>
                        </div>
                    </nav>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-8 is-offset-2">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;nav class=&#x22;level&#x22;&#x3E;
            &#x3C;!-- Left side --&#x3E;
            &#x3C;div class=&#x22;level-left&#x22;&#x3E;
                &#x3C;div class=&#x22;level-item&#x22;&#x3E;
                    &#x3C;p class=&#x22;subtitle is-5&#x22;&#x3E;
                        &#x3C;strong&#x3E;123&#x3C;/strong&#x3E; tasks
                    &#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;level-item&#x22;&#x3E;
                    &#x3C;div class=&#x22;field has-addons&#x22;&#x3E;
                        &#x3C;p class=&#x22;control no-margin-bottom&#x22;&#x3E;
                            &#x3C;input class=&#x22;input&#x22; type=&#x22;text&#x22; placeholder=&#x22;Find a task&#x22;&#x3E;
                        &#x3C;/p&#x3E;
                        &#x3C;p class=&#x22;control&#x22;&#x3E;
                            &#x3C;button class=&#x22;button btn-align accent-btn raised&#x22;&#x3E;
                                Search
                            &#x3C;/button&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        
            &#x3C;!-- Right side --&#x3E;
            &#x3C;div class=&#x22;level-right&#x22;&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-grid is-icon-md&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/p&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-user is-icon-md&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/p&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-picture is-icon-md&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/p&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-check is-icon-md&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/p&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;a class=&#x22;button btn-align accent-btn raised&#x22;&#x3E;New task&#x3C;/a&#x3E;&#x3C;/p&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/nav&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Nav level -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <span class="tag is-secondary">Reworked 3.0.0</span>
                        <h4>Navigation level</h4>
                        <p class="no-padding">A level can easily be turned into a navigation bar. Add the class <code>.nav-level</code> to the <code>.level</code> element. </p>
                    </div>
                </div>
            </div>

            <div class="columns">
                <div class="column is-10 is-offset-1">
                    <nav class="level nav-level">
                        <!-- Left side -->
                        <div class="level-left">
                            <div class="level-item">
                                <p class="subtitle is-5">
                                    <strong>123</strong> tasks
                                </p>
                            </div>
                            <div class="level-item">
                                <div class="field has-addons">
                                    <p class="control no-margin-bottom">
                                        <input class="input" type="text" placeholder="Find a task">
                                    </p>
                                    <p class="control">
                                        <button class="button btn-align accent-btn raised">
                                            Search
                                        </button>
                                    </p>
                                </div>
                            </div>
                        </div>

                        <!-- Right side -->
                        <div class="level-right">
                            <p class="level-item"><i class="sl sl-icon-grid is-icon-md"></i></p>
                            <p class="level-item"><i class="sl sl-icon-user is-icon-md"></i></p>
                            <p class="level-item"><i class="sl sl-icon-picture is-icon-md"></i></p>
                            <p class="level-item"><i class="sl sl-icon-check is-icon-md"></i></p>
                            <p class="level-item"><a class="button btn-align accent-btn raised">New task</a></p>
                        </div>
                    </nav>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-8 is-offset-2">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;nav class=&#x22;level nav-level&#x22;&#x3E;
            &#x3C;!-- Left side --&#x3E;
            &#x3C;div class=&#x22;level-left&#x22;&#x3E;
                &#x3C;div class=&#x22;level-item&#x22;&#x3E;
                    &#x3C;p class=&#x22;subtitle is-5&#x22;&#x3E;
                        &#x3C;strong&#x3E;123&#x3C;/strong&#x3E; tasks
                    &#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;level-item&#x22;&#x3E;
                    &#x3C;div class=&#x22;field has-addons&#x22;&#x3E;
                        &#x3C;p class=&#x22;control no-margin-bottom&#x22;&#x3E;
                            &#x3C;input class=&#x22;input&#x22; type=&#x22;text&#x22; placeholder=&#x22;Find a task&#x22;&#x3E;
                        &#x3C;/p&#x3E;
                        &#x3C;p class=&#x22;control&#x22;&#x3E;
                            &#x3C;button class=&#x22;button btn-align accent-btn raised&#x22;&#x3E;
                                Search
                            &#x3C;/button&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        
            &#x3C;!-- Right side --&#x3E;
            &#x3C;div class=&#x22;level-right&#x22;&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-grid is-icon-md&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/p&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-user is-icon-md&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/p&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-picture is-icon-md&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/p&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-check is-icon-md&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/p&#x3E;
                &#x3C;p class=&#x22;level-item&#x22;&#x3E;&#x3C;a class=&#x22;button btn-align accent-btn raised&#x22;&#x3E;New task&#x3C;/a&#x3E;&#x3C;/p&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/nav&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Centered level -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Centered level</h4>
                        <p class="no-padding">Level items can be centered. To achieve this, remove <code>.level-left</code> and <code>.level-right</code> wrappers. All level items must be direct children of the <code>.level</code> element.</p>
                    </div>
                </div>
            </div>

            <div class="columns">
                <div class="column is-10 is-offset-1">
                    <nav class="level centered-level">
                        <p class="level-item has-text-centered">
                            <a class="link is-info">Projects</a>
                        </p>
                        <p class="level-item has-text-centered">
                            <a class="link is-info">Tasks</a>
                        </p>
                        <p class="level-item has-text-centered">
                            <img class="height-30" src="assets/img/logos/logo-grayscale.png" alt="">
                        </p>
                        <p class="level-item has-text-centered">
                            <a class="link is-info">Team</a>
                        </p>
                        <p class="level-item has-text-centered">
                            <a class="link is-info">Documents</a>
                        </p>
                    </nav>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-8 is-offset-2">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;nav class=&#x22;level centered-level&#x22;&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;Projects&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;Tasks&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;img src=&#x22;assets/img/logos/logo-grayscale.png&#x22; alt=&#x22;&#x22;&#x3E;
            &#x3C;/p&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;Team&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;Documents&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
        &#x3C;/nav&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Mobile level -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Mobile level</h4>
                        <p class="no-padding">The level's inline layout can be kept on mobile, instead of the stacking behaviour. Simply add the modifier <code>.is-mobile</code> to the <code>.level</code> element. Reduce your screen size to see it in action.</p>
                    </div>
                </div>
            </div>

            <div class="columns">
                <div class="column is-4 is-offset-4">
                    <nav class="level is-mobile">
                        <p class="level-item has-text-centered">
                            <a class="link is-info"><i class="fa fa-facebook fa-2x"></i></a>
                        </p>
                        <p class="level-item has-text-centered">
                            <a class="link is-info"><i class="fa fa-twitter fa-2x"></i></a>
                        </p>
                        <p class="level-item has-text-centered">
                            <a class="link is-info"><i class="fa fa-linkedin fa-2x"></i></a>
                        </p>
                        <p class="level-item has-text-centered">
                            <a class="link is-info"><i class="fa fa-dribbble fa-2x"></i></a>
                        </p>
                        <p class="level-item has-text-centered">
                            <a class="link is-info"><i class="fa fa-github fa-2x"></i></a>
                        </p>
                    </nav>
                </div>
            </div>

            <div class="columns is-vcentered pb-60">
                <div class="column is-8 is-offset-2">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;nav class=&#x22;level is-mobile&#x22;&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;&#x3C;i class=&#x22;fa fa-facebook fa-2x&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;&#x3C;i class=&#x22;fa fa-twitter fa-2x&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;&#x3C;i class=&#x22;fa fa-linkedin fa-2x&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;&#x3C;i class=&#x22;fa fa-dribbble fa-2x&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
            &#x3C;p class=&#x22;level-item has-text-centered&#x22;&#x3E;
                &#x3C;a class=&#x22;link is-info&#x22;&#x3E;&#x3C;i class=&#x22;fa fa-github fa-2x&#x22;&#x3E;&#x3C;/i&#x3E;&#x3C;/a&#x3E;
            &#x3C;/p&#x3E;
        &#x3C;/nav&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="section">
        <div class="container">
            <div id="special-lists" class="section-title-wrapper has-text-centered pt-60">
                <div class="bg-number">3</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    special lists
                </h2>
            </div>

            <!-- User list -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>User list</h4>
                        <p class="no-padding">You can use lists to display other types of content. This is a user availability list for example.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <ul class="user-list">
                        <li>
                            <div class="user-list-avatar">
                                <img src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/terry.jpg">
                            </div>
                            <div class="user-list-info">
                                <div class="name">Terry Daniels</div>
                                <div class="position">CEO</div>
                            </div>
                            <div class="user-list-status is-online"></div>
                        </li>
                        <li>
                            <div class="user-list-avatar">
                                <img src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/carolin.png">
                            </div>
                            <div class="user-list-info">
                                <div class="name">Marjory Cambell</div>
                                <div class="position">CFO</div>
                            </div>
                            <div class="user-list-status is-busy"></div>
                        </li>
                        <li>
                            <div class="user-list-avatar">
                                <img src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/ben.jpg">
                            </div>
                            <div class="user-list-info">
                                <div class="name">Kevin Smith</div>
                                <div class="position">Software Engineer</div>
                            </div>
                            <div class="user-list-status is-offline"></div>
                        </li>
                        <li>
                            <div class="user-list-avatar">
                                <img src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/melany.jpg">
                            </div>
                            <div class="user-list-info">
                                <div class="name">Melany Wright</div>
                                <div class="position">Sales Manager</div>
                            </div>
                            <div class="user-list-status is-online"></div>
                        </li>
                    </ul>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-8 is-offset-2">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;ul class=&#x22;user-list&#x22;&#x3E;
            &#x3C;li&#x3E;
                &#x3C;div class=&#x22;user-list-avatar&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/avatars/terry.jpg&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;user-list-info&#x22;&#x3E;
                    &#x3C;div class=&#x22;name&#x22;&#x3E;Terry Daniels&#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;position&#x22;&#x3E;CEO&#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;user-list-status is-online&#x22;&#x3E;&#x3C;/div&#x3E;
            &#x3C;/li&#x3E;
            &#x3C;li&#x3E;
                &#x3C;div class=&#x22;user-list-avatar&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/avatars/carolin.png&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;user-list-info&#x22;&#x3E;
                    &#x3C;div class=&#x22;name&#x22;&#x3E;Marjory Cambell&#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;position&#x22;&#x3E;CFO&#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;user-list-status is-busy&#x22;&#x3E;&#x3C;/div&#x3E;
            &#x3C;/li&#x3E;
            &#x3C;li&#x3E;
                &#x3C;div class=&#x22;user-list-avatar&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/avatars/ben.jpg&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;user-list-info&#x22;&#x3E;
                    &#x3C;div class=&#x22;name&#x22;&#x3E;Kevin Smith&#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;position&#x22;&#x3E;Software Engineer&#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;user-list-status is-offline&#x22;&#x3E;&#x3C;/div&#x3E;
            &#x3C;/li&#x3E;
            &#x3C;li&#x3E;
                &#x3C;div class=&#x22;user-list-avatar&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/avatars/melany.jpg&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;user-list-info&#x22;&#x3E;
                    &#x3C;div class=&#x22;name&#x22;&#x3E;Melany Wright&#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;position&#x22;&#x3E;Sales Manager&#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;user-list-status is-online&#x22;&#x3E;&#x3C;/div&#x3E;
            &#x3C;/li&#x3E;
        &#x3C;/ul&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Media list -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Media list</h4>
                        <p class="no-padding">This is another example of a user list. It is based on the Bulma media object element.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4">
                    <ul class="user-media-list">
                        <li>
                            <div class="media">
                                <figure class="media-left">
                                    <p class="image is-48x48">
                                        <img class="img-circle" src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/terry.jpg">
                                        <span class="avatar-status is-online"></span>
                                    </p>
                                </figure>
                                <div class="media-content">
                                    <div class="content">
                                        <p>
                                            <span>Terry Daniels</span>
                                            <br>
                                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="media">
                                <figure class="media-left">
                                    <p class="image is-48x48">
                                        <img class="img-circle" src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/carolin.png">
                                        <span class="avatar-status is-busy"></span>
                                    </p>
                                </figure>
                                <div class="media-content">
                                    <div class="content">
                                        <p>
                                            <span>Marjory Cambell</span>
                                            <br>
                                                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sinum garet et setus.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="media">
                                <figure class="media-left">
                                    <p class="image is-48x48">
                                        <img class="img-circle" src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/ben.jpg">
                                        <span class="avatar-status is-offline"></span>
                                    </p>
                                </figure>
                                <div class="media-content">
                                    <div class="content">
                                        <p>
                                            <span>Kevin Smith</span>
                                            <br>
                                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="media">
                                <figure class="media-left">
                                    <p class="image is-48x48">
                                        <img class="img-circle" src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/melany.jpg">
                                        <span class="avatar-status is-online"></span>
                                    </p>
                                </figure>
                                <div class="media-content">
                                    <div class="content">
                                        <p>
                                            <span>Melany Wright</span>
                                            <br>
                                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>

            <div class="columns is-vcentered pb-60">
                <div class="column is-8 is-offset-2">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-lists.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;ul class=&#x22;user-media-list&#x22;&#x3E;
            &#x3C;li&#x3E;
                &#x3C;article class=&#x22;media&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;p class=&#x22;image is-48x48&#x22;&#x3E;
                            &#x3C;img class=&#x22;img-circle&#x22; src=&#x22;assets/img/avatars/terry.jpg&#x22;&#x3E;
                            &#x3C;span class=&#x22;avatar-status is-online&#x22;&#x3E;&#x3C;/span&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span&#x3E;Terry Daniels&#x3C;/span&#x3E;
                                &#x3C;br&#x3E;
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
            &#x3C;/li&#x3E;
            &#x3C;li&#x3E;
                &#x3C;article class=&#x22;media&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;p class=&#x22;image is-48x48&#x22;&#x3E;
                            &#x3C;img class=&#x22;img-circle&#x22; src=&#x22;assets/img/avatars/carolin.png&#x22;&#x3E;
                            &#x3C;span class=&#x22;avatar-status is-busy&#x22;&#x3E;&#x3C;/span&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span&#x3E;Marjory Cambell&#x3C;/span&#x3E;
                                &#x3C;br&#x3E;
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sinum garet et setus.
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
            &#x3C;/li&#x3E;
            &#x3C;li&#x3E;
                &#x3C;article class=&#x22;media&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;p class=&#x22;image is-48x48&#x22;&#x3E;
                            &#x3C;img class=&#x22;img-circle&#x22; src=&#x22;assets/img/avatars/ben.jpg&#x22;&#x3E;
                            &#x3C;span class=&#x22;avatar-status is-offline&#x22;&#x3E;&#x3C;/span&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span&#x3E;Kevin Smith&#x3C;/span&#x3E;
                                &#x3C;br&#x3E;
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
            &#x3C;/li&#x3E;
            &#x3C;li&#x3E;
                &#x3C;article class=&#x22;media&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;p class=&#x22;image is-48x48&#x22;&#x3E;
                            &#x3C;img class=&#x22;img-circle&#x22; src=&#x22;assets/img/avatars/melany.jpg&#x22;&#x3E;
                            &#x3C;span class=&#x22;avatar-status is-online&#x22;&#x3E;&#x3C;/span&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span&#x3E;Melany Wright&#x3C;/span&#x3E;
                                &#x3C;br&#x3E;
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
            &#x3C;/li&#x3E;
        &#x3C;/ul&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>

    <!-- Dark footer -->
    <footer id="dark-footer" class="footer footer-dark">
        <div class="container">
            <div class="columns">
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Product</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-lists.php#">Discover features</a></li>
                            <li><a href="_components-basicui-lists.php#">CMS integration</a></li>
                            <li><a href="_components-basicui-lists.php#">Customers</a></li>
                            <li><a href="_components-basicui-lists.php#">Weekly sessions</a></li>
                            <li><a href="_components-basicui-lists.php#">Free trials and v1</a></li>
                            <li><a href="_components-basicui-lists.php#">What's next ?</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Company</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-lists.php#">About us</a></li>
                            <li><a href="_components-basicui-lists.php#">About security</a></li>
                            <li><a href="_components-basicui-lists.php#">User guide</a></li>
                            <li><a href="_components-basicui-lists.php#">Terms of service</a></li>
                            <li><a href="_components-basicui-lists.php#">Terms of website use</a></li>
                            <li><a href="_components-basicui-lists.php#">Privacy policy</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Learning</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-lists.php#">Resources</a></li>
                            <li><a href="_components-basicui-lists.php#">Blog</a></li>
                            <li><a href="_components-basicui-lists.php#">FAQ</a></li>
                            <li><a href="_components-basicui-lists.php#">API documentation</a></li>
                            <li><a href="_components-basicui-lists.php#">Admin guide</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-logo">
                            <img class="switcher-logo-w" src="assets/img/logos/logo/bulkit-core-w.svg" alt="">
                        </div>
                        <div class="footer-header">
                            <nav class="level is-mobile">
                                <div class="level-left level-social">
                                    <a href="_components-basicui-lists.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-facebook"></i></span>
                                    </a>
                                    <a href="_components-basicui-lists.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-twitter"></i></span>
                                    </a>
                                    <a href="_components-basicui-lists.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-linkedin"></i></span>
                                    </a>
                                    <a href="_components-basicui-lists.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-dribbble"></i></span>
                                    </a>
                                    <a href="_components-basicui-lists.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-github"></i></span>
                                    </a>
                                </div>
                            </nav>
                        </div>
                        <div class="copyright">
                            <span class="moto light-text">Designed and coded with <i class="fa fa-heart color-red"></i> by CSS Ninja.</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </footer>
    <!-- /Dark footer -->
    <!-- Side navigation -->
    <div class="side-navigation-menu">
        <!-- Categories menu -->
        <div class="category-menu-wrapper">
            <!-- Menu -->
            <ul class="categories">
                <li class="square-logo"><img src="assets/img/logos/square-white.svg" alt=""></li>
                <li class="category-link is-active" data-navigation-menu="v1-pages"><i class="sl sl-icon-layers"></i></li>
                <li class="category-link" data-navigation-menu="onepagers"><i class="sl sl-icon-docs"></i></li>
                <li class="category-link" data-navigation-menu="components"><i class="sl sl-icon-grid"></i></li>
            </ul>
            <!-- Menu -->

            <ul class="author">
                <li>
                    <!-- Theme author -->
                    <a href="https://cssninja.io" target="_blank">
                        <img class="main-menu-author" src="assets/img/logos/cssninja.svg" alt="">
                    </a>
                </li>
            </ul>
        </div>
        <!-- /Categories menu -->

        <!-- Navigation menu -->
        <div id="v1-pages" class="navigation-menu-wrapper animated preFadeInRight fadeInRight">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>V1 pages</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation Body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">weekend</span>Agency kit</a>
                    <ul>
                        <li><a class="is-submenu" href="agency-home.php">Homepage</a></li>
                        <li><a class="is-submenu" href="agency-about.php">About</a></li>
                        <li><a class="is-submenu" href="agency-portfolio.php">Portfolio</a></li>
                        <li><a class="is-submenu" href="agency-contact.php">Contact</a></li>
                        <li><a class="is-submenu" href="agency-blog.php">Blog</a></li>
                        <li><a class="is-submenu" href="agency-post-nosidebar.php">Post no sidebar</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">wb_incandescent</span>Startup kit</a>
                    <ul>
                        <li><a class="is-submenu" href="startup-landing.php">Homepage</a></li>
                        <li><a class="is-submenu" href="startup-about.php">About</a></li>
                        <li><a class="is-submenu" href="startup-product.php">Product</a></li>
                        <li><a class="is-submenu" href="startup-contact.php">Contact</a></li>
                        <li><a class="is-submenu" href="startup-login.php">Login</a></li>
                        <li><a class="is-submenu" href="startup-signup.php">Sign up</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">apps</span>Landing kit 1</a>
                    <ul>
                        <li><a class="is-submenu" href="kit1-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit1-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit1-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit1-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit1-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit1-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit1-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit1-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit1-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit1-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">timelapse</span>Landing kit 2</a>
                    <ul>
                        <li><a class="is-submenu" href="kit2-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit2-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit2-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit2-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit2-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit2-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit2-landing-7.php">Landing page v7</a></li>
                        <li><a class="is-submenu" href="kit2-landing-8.php">Landing page v8</a></li>
                        <li><a class="is-submenu" href="kit2-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit2-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit2-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit2-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">assistant</span>Landing kit 3</a>
                    <ul>
                        <li><a class="is-submenu" href="kit3-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit3-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit3-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit3-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit3-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit3-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">brightness_2</span>Landing kit 4</a>
                    <ul>
                        <li><a class="is-submenu" href="kit4-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit4-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit4-help.php">Help center</a></li>
                        <li><a class="is-submenu" href="kit4-help-category.php">Help category</a></li>
                        <li><a class="is-submenu" href="kit4-help-article.php">Help article</a></li>
                        <li><a class="is-submenu" href="kit4-signup.php">Sign Up</a></li>
                        <li><a class="is-submenu" href="kit4-login.php">Login</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">toys</span>Landing kit 5</a>
                    <ul>
                        <li><a class="is-submenu" href="kit5-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit5-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit5-pricing.php">Pricing Page</a></li>
                        <li><a class="is-submenu" href="kit5-login.php">Login / Signup</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">mouse</span>Landing kit 6</a>
                    <ul>
                        <li><a class="is-submenu" href="kit6-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit6-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit6-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">work</span>Landing kit
                        7</a>
                    <ul>
                        <li><a class="is-submenu" href="kit7-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit7-landing.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit7-authentication.php">Auth page</a></li>
                        <li><a class="is-submenu" href="kit7-pricing.php">Pricing page</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-lists.php#">
                        <span class="material-icons">public</span>Landing kit 8</a>
                    <ul>
                        <li><a class="is-submenu" href="kit8-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit8-landing-1.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit8-landing-2.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit8-landing-3.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit8-landing-4.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit8-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit8-jobs.php">Jobs page</a></li>
                        <li><a class="is-submenu" href="kit8-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit8-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-lists.php#">
                        <span class="material-icons">assessment</span>Landing kit 9</a>
                    <ul>
                        <li><a class="is-submenu" href="kit9-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit9-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit9-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit9-landing-4.php">Landing page v4</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-lists.php#">
                        <span class="material-icons">account_balance</span>Landing kit 10</a>
                    <ul>
                        <li><a class="is-submenu" href="kit10-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit10-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit10-landing-3.php">Landing page v3</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-lists.php#">
                        <span class="material-icons">shop</span>Landing kit 11</a>
                    <ul>
                        <li><a class="is-submenu" href="kit11-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit11-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit11-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit11-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit11-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit11-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit11-landing-7.php">Landing page v7</a></li>
                        <li><a class="is-submenu" href="kit11-landing-8.php">Landing page v8</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-lists.php#">
                        <span class="material-icons">stars</span>Landing kit 12</a>
                    <ul>
                        <li><a class="is-submenu" href="kit12-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit12-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit12-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit12-landing-4.php">Landing page v4</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-lists.php#">
                        <span class="material-icons">new_releases</span>Landing kit 13</a>
                    <ul>
                        <li><a class="is-submenu" href="kit13-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit13-landing-2.php">Landing page v2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-lists.php#">
                        <span class="material-icons">code</span>Landing kit 14</a>
                    <ul>
                        <li><a class="is-submenu" href="kit14-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit14-landing-2.php">Landing page v2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-lists.php#">
                        <span class="material-icons">settings</span>Landing kit 15</a>
                    <ul>
                        <li><a class="is-submenu" href="kit15-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit15-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit15-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit15-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit15-about.php">About page</a></li>
                        <li><a class="is-submenu" href="kit15-contact.php">Contact page</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- Navigation menu -->
        <div id="onepagers" class="navigation-menu-wrapper animated preFadeInRight fadeInRight is-hidden">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>Sub Pages</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-lists.php#"><span class="material-icons">domain</span>Company</a>
                    <ul>
                        <li><a class="is-submenu" href="about-page-1.php">About v1</a></li>
                        <li><a class="is-submenu" href="about-page-2.php">About v2</a></li>
                        <li><a class="is-submenu" href="case-study-1.php">Case Study v1</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-lists.php#"><span class="material-icons">call</span>Contact</a>
                    <ul>
                        <li><a class="is-submenu" href="contact-page-1.php">Contact v1</a></li>
                        <li><a class="is-submenu" href="contact-page-2.php">Contact v2</a></li>
                        <li><a class="is-submenu" href="contact-page-3.php">Contact v3</a></li>
                        <li><a class="is-submenu" href="contact-page-4.php">Contact v4</a></li>
                        <li><a class="is-submenu" href="contact-page-5.php">Contact v5</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-lists.php#"><span class="material-icons">book</span>Blog</a>
                    <ul>
                        <li><a class="is-submenu" href="blog-posts-full.php">Posts List v1</a></li>
                        <li><a class="is-submenu" href="blog-posts-full-alt.php">Posts List v2</a></li>
                        <li><a class="is-submenu" href="blog-posts-side.php">Posts List v3</a></li>
                        <li><a class="is-submenu" href="blog-posts-side-alt.php">Posts List v4</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-full.php">Posts Grid v1</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-full-masonry.php">Posts Grid v2</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-side.php">Posts Grid v3</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-side-masonry.php">Posts Grid v4</a></li>
                        <li><a class="is-submenu" href="blog-single-full.php">Single Post V1</a></li>
                        <li><a class="is-submenu" href="blog-single-side.php">Single Post V2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link has-new" href="_components-basicui-lists.php#"><span class="material-icons">highlight</span>Error Pages</a>
                    <ul>
                        <li><a class="is-submenu" href="error-1.php">Error v1</a></li>
                        <li><a class="is-submenu" href="error-2.php">Error v2</a></li>
                        <li><a class="is-submenu" href="error-3.php">Error v3</a></li>
                        <li><a class="is-submenu" href="error-4.php">Error v4</a></li>
                        <li><a class="is-submenu" href="error-5.php">Error v5</a></li>
                        <li><a class="is-submenu" href="error-6.php">Error v6</a></li>
                        <li><a class="is-submenu" href="error-7.php">Error v7</a></li>
                        <li><a class="is-submenu" href="error-8.php">Error v8</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- Navigation menu -->
        <div id="components" class="navigation-menu-wrapper animated preFadeInRight fadeInRight is-hidden">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>Components</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">view_quilt</span>Layout</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-layout-grid.php">Grid system</a></li>
                        <li><a class="is-submenu" href="_components-layout-video.php">Video background</a></li>
                        <li><a class="is-submenu" href="_components-layout-parallax.php">Parallax</a></li>
                        <li><a class="is-submenu" href="_components-layout-headers.php">Headers</a></li>
                        <li><a class="is-submenu" href="_components-layout-footers.php">Footers</a></li>
                        <li><a class="is-submenu" href="_components-layout-typography.php">Typography</a></li>
                        <li><a class="is-submenu" href="_components-layout-colors.php">Colors</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">subject</span>Navbars</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-layout-navbar-fade-light.php">Fade light</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-fade-dark.php">Fade dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-light.php">Static
                                light</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-dark.php">Static
                                dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-solid.php">Static
                                solid</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-double-dark.php">Double
                                dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-double-light.php">Double
                                light</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">view_stream</span>Sections</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-sections-features.php">Features</a></li>
                        <li><a class="is-submenu" href="_components-sections-pricing.php">Pricing</a></li>
                        <li><a class="is-submenu" href="_components-sections-team.php">Team</a></li>
                        <li><a class="is-submenu" href="_components-sections-testimonials.php">Testimonials</a></li>
                        <li><a class="is-submenu" href="_components-sections-clients.php">Clients</a></li>
                        <li><a class="is-submenu" href="_components-sections-counters.php">Counters</a></li>
                        <li><a class="is-submenu" href="_components-sections-carousel.php">Carousel</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">playlist_add_check</span>Basic
                        UI</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-basicui-cards.php">Cards</a></li>
                        <li><a class="is-submenu" href="_components-basicui-buttons.php">Buttons</a></li>
                        <li><a class="is-submenu" href="_components-basicui-dropdowns.php">Dropdowns</a></li>
                        <li><a class="is-submenu" href="_components-basicui-lists.php">Lists</a></li>
                        <li><a class="is-submenu" href="_components-basicui-modals.php">Modals</a></li>
                        <li><a class="is-submenu" href="_components-basicui-tabs.php">Tabs & pills</a></li>
                        <li><a class="is-submenu" href="_components-basicui-accordion.php">Accordions</a></li>
                        <li><a class="is-submenu" href="_components-basicui-badges.php">Badges & labels</a></li>
                        <li><a class="is-submenu" href="_components-basicui-popups.php">Popups</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">playlist_add</span>Advanced
                        UI</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-advancedui-tables.php">Tables</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-timeline.php">Timeline</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-boxes.php">Boxes</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-messages.php">Messages</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-megamenu.php">Megamenu</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-calendar.php">Calendar</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">check_box</span>Forms</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-forms-inputs.php">Inputs</a></li>
                        <li><a class="is-submenu" href="_components-forms-controls.php">Controls</a></li>
                        <li><a class="is-submenu" href="_components-forms-layouts.php">Form layouts</a></li>
                        <li><a class="is-submenu" href="_components-forms-steps.php">Step forms</a></li>
                        <li><a class="is-submenu" href="_components-forms-uploader.php">Uploader</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-lists.php#"><span class="material-icons">adjust</span>Icons</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-icons-im.php">Icons Mind</a></li>
                        <li><a class="is-submenu" href="_components-icons-sl.php">Simple Line Icons</a></li>
                        <li><a class="is-submenu" href="_components-icons-fa.php">Font Awesome</a></li>
                        <li><a class="is-submenu" href="https://material.io/icons/" target="_blank">Material Icons</a></li>
                        <li><a class="is-submenu" href="_components-extensions-iconpicker.php">Icon Picker</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- /Navigation menu -->
    </div>
    <!-- /Side navigation -->
    <!-- Back To Top Button -->
    <div id="backtotop"><a href="_components-basicui-lists.php#"></a></div> <!-- Concatenated jQuery and plugins -->
    <script src="assets/js/app.js"></script>

    <!-- Bulkit js -->
    <script src="assets/js/functions.js"></script>
    <script src="assets/js/main.js"></script>
</body>

</html>