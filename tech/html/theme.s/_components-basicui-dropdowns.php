<!DOCTYPE html>
<html lang="en">

<head>
    <!-- Required Meta Tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta http-equiv="x-ua-compatible" content="ie=edge">

    <title> Bulkit :: Dropdowns</title>
    <link rel="icon" type="image/png" href="assets/img/favicon.png" />

    <!--Core CSS -->
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="assets/css/bulma.css">
    <link rel="stylesheet" href="assets/css/app.css">
    <link rel="stylesheet" href="assets/css/core.css">
</head>

<body id="component-page">
    <div class="pageloader"></div>
    <div class="infraloader is-active"></div>
    <!-- Hero and nav -->
    <div class="hero is-relative is-theme-primary">

        <nav class="navbar navbar-wrapper navbar-light is-transparent is-static">
            <div class="container">
                <!-- Brand -->
                <div class="navbar-brand">
                    <a class="navbar-item" href="index.php">
                        <img src="assets/img/logos/bulkit-white.svg" alt="">
                    </a>

                    <!-- Sidebar Trigger -->
                    <a id="navigation-trigger" class="navbar-item hamburger-btn" href="javascript:void(0);">
                        <span class="menu-toggle">
                                <span class="icon-box-toggle">
                                    <span class="rotate">
                                        <i class="icon-line-top"></i>
                                        <i class="icon-line-center"></i>
                                        <i class="icon-line-bottom"></i>
                                    </span>
                        </span>
                        </span>
                    </a>

                    <!-- Responsive toggle -->
                    <div class="custom-burger" data-target="">
                        <a id="" class="responsive-btn" href="javascript:void(0);">
                            <span class="menu-toggle">
                                    <span class="icon-box-toggle">
                                        <span class="rotate">
                                            <i class="icon-line-top"></i>
                                            <i class="icon-line-center"></i>
                                            <i class="icon-line-bottom"></i>
                                        </span>
                            </span>
                            </span>
                        </a>
                    </div>
                    <!-- /Responsive toggle -->
                </div>

                <!-- Navbar menu -->
                <div class="navbar-menu">
                    <!-- Navbar Start -->
                    <div class="navbar-start">
                        <!-- Dropdown -->
                        <div class="navbar-item has-dropdown is-hoverable is-mega">
                            <div class="navbar-link">
                                Menu
                            </div>
                            <div class="navbar-dropdown is-megamenu animated prefadeInUp fadeInUp" data-style="width: 18rem;">
                                <div class="mega-inner">
                                    <div class="container">
                                        <div class="columns">
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Landing Kits</h2>
                                                <div class="scrollable-container" data-simplebar>
                                                    <a class="navbar-item is-flex" href="kit1-landing.php">
                                                        <i class="im im-icon-Navigation-RightWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 1</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit2-landing.php">
                                                        <i class="im im-icon-Split-Horizontal"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 2</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit3-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 3</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit4-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 4</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit5-landing.php">
                                                        <i class="im im-icon-Love-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 5</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit6-landing.php">
                                                        <i class="im im-icon-Two-Windows"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 6</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit7-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 7</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit5-landing.php">
                                                        <i class="im im-icon-Love-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 5</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit6-landing.php">
                                                        <i class="im im-icon-Two-Windows"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 6</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit7-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 7</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit8-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 8</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit9-landing.php">
                                                        <i class="im im-icon-Split-FourSquareWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 9</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit10-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 10</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit11-landing.php">
                                                        <i class="im im-icon-Favorite-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 11</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit12-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 12</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit13-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 13</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit14-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 14</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit15-landing.php">
                                                        <i class="im im-icon-Split-FourSquareWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 15</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Utility Pages</h2>
                                                <div class="scrollable-container" data-simplebar="">
                                                    <a class="navbar-item is-flex" href="about-page-1.php">
                                                        <i class="im im-icon-Building"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">ABOUT</small>
                                                            </p>
                                                            <p>About v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="about-page-2.php">
                                                        <i class="im im-icon-Building"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">ABOUT</small>
                                                            </p>
                                                            <p>About v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="about-page-2.php">
                                                        <i class="im im-icon-Structure"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CASE STUDY</small>
                                                            </p>
                                                            <p>Case Study v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-1.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-2.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-3.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-4.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-5.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v5</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-full-alt.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-side-alt.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-full-masonry.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-side-masonry.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-single-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Single Post v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-single-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Single Post v2</p>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Themed Kits</h2>
                                                <a class="navbar-item is-flex" href="agency-home.php">
                                                    <i class="im im-icon-Post-Office"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">AGENCY</small>
                                                        </p>
                                                        <p>Agency landing</p>
                                                    </div>
                                                </a>
                                                <a class="navbar-item is-flex" href="startup-home.php">
                                                    <i class="im im-icon-Factory"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">STARTUP</small>
                                                        </p>
                                                        <p>Startup landing</p>
                                                    </div>
                                                </a>
                                                <a class="navbar-item is-flex" href="error-1.php">
                                                    <i class="im im-icon-Coffin"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">ERROR KIT</small>
                                                        </p>
                                                        <p>Error Pages</p>
                                                    </div>
                                                </a>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">More goodies</h2>
                                                <p class="is-cta">As a Bulkit customer, you get some free goodies on Css Ninja.</p>
                                                <p class="is-cta">
                                                    <a href="https://cssninja.io" target="_blank" class="button btn-align mega-btn primary-btn">Learn more</a>
                                                </p>
                                            </div>
                                        </div>
                                    </div>

                                    <hr class="navbar-divider">
                                    <div class="navbar-item is-footer is-hidden-mobile">
                                        <div class="navbar-content">
                                            <div class="level is-mobile">
                                                <div class="level-left">
                                                    <div class="level-item">
                                                        <strong>Follow us</strong>
                                                    </div>
                                                </div>
                                                <div class="level-right">
                                                    <div class="level-item">
                                                        <a class="social-link" href="_components-basicui-dropdowns.php#"><i class="fa fa-twitter"></i></a>
                                                        <a class="social-link" href="_components-basicui-dropdowns.php#"><i class="fa fa-facebook"></i></a>
                                                        <a class="social-link" href="_components-basicui-dropdowns.php#"><i class="fa fa-behance"></i></a>
                                                        <a class="social-link" href="_components-basicui-dropdowns.php#"><i class="fa fa-github"></i></a>
                                                        <a class="social-link" href="_components-basicui-dropdowns.php#"><i class="fa fa-tumblr"></i></a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="menu-image is-hidden-mobile">
                                        <img src="assets/img/graphics/legacy/megamenu-core.svg" data-base-url="assets/img/graphics/legacy/megamenu" data-extension=".svg" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Mobile dropdown -->
                        <!--div class="mobile-drop is-hidden-desktop">
                                <div class="mobile-drop-trigger">
                                    V1 <i class="material-icons">keyboard_arrow_right</i>
                                </div>
                                <div class="child-menu">
                                    <ul>
                                        <li>
                                            <a href="/agency-home.php">Agency kit</a>
                                        </li>
                                        <li>
                                            <a href="/startup-landing.php">Startup kit</a>
                                        </li>
                                        <li>
                                            <a href="/kit1-landing.php">Landing kit 1</a>
                                        </li>
                                        <li>
                                            <a href="/kit2-landing.php">Landing kit 2</a>
                                        </li>
                                        <li>
                                            <a href="/kit3-landing.php">Landing kit 3</a>
                                        </li>
                                        <li>
                                            <a href="/kit4-landing.php">Landing kit 4</a>
                                        </li>
                                        <li>
                                            <a href="/kit5-landing.php">Landing kit 5</a>
                                        </li>
                                        <li>
                                            <a href="/kit6-landing.php">Landing kit 6</a>
                                        </li>
                                        <li>
                                            <a href="/kit7-landing.php">Landing kit 7</a>
                                        </li>
                                        <li>
                                            <a href="/kit8-landing.php">Landing kit 8</a>
                                        </li>
                                        <li>
                                            <a href="/kit9-landing.php">Landing kit 9</a>
                                        </li>
                                        <li>
                                            <a href="/kit10-landing.php">Landing kit 10</a>
                                        </li>
                                        <li>
                                            <a href="/kit11-landing.php">Landing kit 11</a>
                                        </li>
                                    </ul>
                                </div>                </div-->
                        <!-- Navbar item -->
                        <a class="navbar-item is-slide" href="https://docs.bulkit.cssninja.io/" target="_blank">
                            Docs
                        </a>
                        <!-- Navbar item -->
                        <a class="navbar-item is-slide" href="https://cssninja.io/" target="_blank">
                            Help
                        </a>
                    </div>

                    <!-- Navbar end -->
                    <div class="navbar-end">
                        <!-- Signup button -->
                        <div class="navbar-item">
                            <a id="#signup-btn" href="https://1.envato.market/c/1288816/275988/4415?subId1=button&subId2=banner&subId3=https%3A%2F%2Fthemeforest.net%2Fitem%2Fbulkit-agency-startup-and-saas-template%2F21278677&u=https%3A%2F%2Fthemeforest.net%2Fitem%2Fbulkit-agency-startup-and-saas-template%2F21278677" class="button button-cta btn-outlined is-bold btn-align light-btn rounded raised">
                                Buy now
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </nav>
        <!-- Hero image -->
        <div id="main-hero" class="hero-body">
            <div class="container has-text-centered">
                <div class="columns is-vcentered">
                    <div class="column is-4 is-offset-1 has-text-left">
                        <h1 class="title components-title is-1">
                            Dropdowns
                        </h1>
                        <h2 class="subtitle is-4 components-subtitle">
                            Dropdown styles and variations.
                        </h2>
                        <p class="components-cta">
                            <a href="_components-basicui-dropdowns.php#hover-dropdowns" class="button button-cta btn-align light-btn btn-outlined is-bold rounded">
                                Get Started
                            </a>
                        </p>
                    </div>
                    <div class="column is-offset-1 has-text-centered is-hidden-mobile">
                        <div id="dropdowns-ill" class="vivus-svg"></div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Scrollnav -->
    <div id="scrollnav" class="scroll-nav-wrapper">
        <div class="container">
            <div class="tabs scrollnav-tabs is-centered">
                <ul>
                    <li class="scrollnav-item is-active"><a href="_components-basicui-dropdowns.php#hover-dropdowns">Hover dropdowns</a></li>
                    <li class="scrollnav-item"><a href="_components-basicui-dropdowns.php#click-dropdowns">Click dropdowns</a></li>
                    <li class="scrollnav-item"><a href="_components-basicui-dropdowns.php#drop-panels">Drop panels</a></li>
                </ul>
            </div>
        </div>
    </div>

    <div class="section">
        <div class="container">
            <div id="hover-dropdowns" class="section-title-wrapper has-text-centered">
                <div class="bg-number">1</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Hover dropdowns
                </h2>
            </div>

            <!-- Basic hover dropdowns -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Basic dropdown</h4>
                        <p class="no-padding">If you dont want to use any javascript for your dropdowns, Bulkit ships with handy hover dropdowns. Any element can be a hover dropdown trigger. You just need to add the <code>.is-drop</code> class to it. Then insert the dropdown structure inside the trigger element. Note that it is better if the trigger element has a defined width, like a button, else your dropdown might not behave as expected. Check the code sample for more details.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3 has-text-centered">
                    <div class="button btn-align has-icon-right is-drop">Dropdown <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut">
                                <ul>
                                    <li>Download invoice</li>
                                    <li>Send info</li>
                                    <li>Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Icon hover dropdowns -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Dropdown with icons</h4>
                        <p class="no-padding">A hover dropdown menu items can have icons on the left. Simply add the icons you want inside the item markup. Check the code example.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 is-offset-4 has-text-centered">
                    <div class="button btn-align has-icon-right primary-btn btn-outlined is-drop">Dropdown <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut">
                                <ul>
                                    <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                    <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                    <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-dropdowns.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;button btn-align has-icon-right primary-btn btn-outlined is-drop&#x22;&#x3E;Dropdown &#x3C;i class=&#x22;sl sl-icon-arrow-down is-icon-xs&#x22;&#x3E;&#x3C;/i&#x3E;
            &#x3C;div class=&#x22;dropContain&#x22;&#x3E;
                &#x3C;div class=&#x22;dropOut&#x22;&#x3E;
                    &#x3C;ul&#x3E;
                        &#x3C;li&#x3E;&#x3C;i class=&#x22;drop-icon sl sl-icon-cloud-download&#x22;&#x3E;&#x3C;/i&#x3E; Download invoice&#x3C;/li&#x3E;
                        &#x3C;li&#x3E;&#x3C;i class=&#x22;drop-icon sl sl-icon-paper-plane&#x22;&#x3E;&#x3C;/i&#x3E; Send info&#x3C;/li&#x3E;
                        &#x3C;li&#x3E;&#x3C;i class=&#x22;drop-icon sl sl-icon-trash&#x22;&#x3E;&#x3C;/i&#x3E; Delete&#x3C;/li&#x3E;
                    &#x3C;/ul&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Hover positions -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Dropdown positions</h4>
                        <p class="no-padding">Hover dropdowns can be positioned in 3 ways : left, wich is default, center and right. For a centered dropdown, add the <code>.is-centered</code> class to the <code>.is-drop</code> trigger element. For a right positioned dropdown, use the <code>is-right</code> class.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 has-text-centered">
                    <div class="button btn-align accent-btn btn-outlined has-icon-right is-drop">Left <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut">
                                <ul>
                                    <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                    <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                    <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="column is-4 has-text-centered">
                    <div class="button btn-align danger-btn btn-outlined has-icon-right is-drop is-centered">Center <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut">
                                <ul>
                                    <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                    <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                    <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>


                <div class="column is-4 has-text-centered">
                    <div class="button btn-align success-btn btn-outlined has-icon-right is-drop is-right">Right <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut">
                                <ul>
                                    <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                    <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                    <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Hover colors -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block has-text-centered">
                        <h4>Hover colors</h4>
                        <p class="no-padding">You can change the dropdown items color on hover. For primary color, add the <code>.is-primary</code> class to the <code>.dropOut</code> element. For secondary color, add the <code>.is-secondary</code> class to the <code>.dropOut</code> element. For accent color, add the <code>.is-accent</code> class to the <code>.dropOut</code> element.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-3 has-text-centered">
                    <div class="button btn-align btn-outlined has-icon-right is-drop">Default <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut">
                                <ul>
                                    <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                    <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                    <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="column is-3 has-text-centered">
                    <div class="button btn-align primary-btn btn-outlined has-icon-right is-drop">Primary <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut is-primary">
                                <ul>
                                    <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                    <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                    <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="column is-3 has-text-centered">
                    <div class="button btn-align secondary-btn btn-outlined has-icon-right is-drop">Secondary <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut is-secondary">
                                <ul>
                                    <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                    <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                    <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>


                <div class="column is-3 has-text-centered">
                    <div class="button btn-align accent-btn btn-outlined has-icon-right is-drop">Accent <i class="sl sl-icon-arrow-down"></i>
                        <div class="dropContain">
                            <div class="dropOut is-accent">
                                <ul>
                                    <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                    <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                    <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Card action -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Card action</h4>
                        <p class="no-padding">Hover dropdowns can be very handy for context menus like in this example. Hover over the small arrow on the right side of the card to make the dropdown appear. Check the code example for more details</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- V1 section -->

                    <div class="flex-card light-bordered hover-inset card-overflow">
                        <div class="flex-card-header">
                            <div class="header-avatar">
                                <img src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/helen.jpg">
                            </div>
                            <div class="header-info">
                                <div class="header-title">Helen Miller</div>
                                <div class="header-subtitle">Sales team</div>
                            </div>
                            <div class="header-control ml-auto is-drop drop-sm">
                                <i class="sl sl-icon-arrow-down"></i>
                                <div class="dropContain">
                                    <div class="dropOut">
                                        <ul>
                                            <li><i class="drop-icon sl sl-icon-cloud-download"></i> Download invoice</li>
                                            <li><i class="drop-icon sl sl-icon-paper-plane"></i> Send info</li>
                                            <li><i class="drop-icon sl sl-icon-trash"></i> Delete</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="card-body is-small">
                            <div class="content">
                                <p class="pb-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                            <div class="has-text-right">
                                <a class="button btn-align btn-outlined accent-btn rounded raised">Follow</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered pb-60">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-dropdowns.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card light-bordered hover-inset card-overflow&#x22;&#x3E;
            &#x3C;div class=&#x22;flex-card-header&#x22;&#x3E;
                &#x3C;div class=&#x22;header-avatar&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/avatars/helen.jpg&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;header-info&#x22;&#x3E;
                    &#x3C;div class=&#x22;header-title&#x22;&#x3E;Helen Miller&#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;header-subtitle&#x22;&#x3E;Sales team&#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;header-control ml-auto is-drop drop-sm&#x22;&#x3E;
                    &#x3C;i class=&#x22;sl sl-icon-arrow-down&#x22;&#x3E;&#x3C;/i&#x3E;
                    &#x3C;div class=&#x22;dropContain&#x22;&#x3E;
                        &#x3C;div class=&#x22;dropOut&#x22;&#x3E;
                            &#x3C;ul&#x3E;
                                &#x3C;li&#x3E;&#x3C;i class=&#x22;drop-icon sl sl-icon-cloud-download&#x22;&#x3E;&#x3C;/i&#x3E; Download invoice&#x3C;/li&#x3E;
                                &#x3C;li&#x3E;&#x3C;i class=&#x22;drop-icon sl sl-icon-paper-plane&#x22;&#x3E;&#x3C;/i&#x3E; Send info&#x3C;/li&#x3E;
                                &#x3C;li&#x3E;&#x3C;i class=&#x22;drop-icon sl sl-icon-trash&#x22;&#x3E;&#x3C;/i&#x3E; Delete&#x3C;/li&#x3E;
                            &#x3C;/ul&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-body is-small&#x22;&#x3E;
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    &#x3C;p class=&#x22;pb-10&#x22;&#x3E;Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.&#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;has-text-right&#x22;&#x3E;
                    &#x3C;a class=&#x22;button btn-align btn-outlined accent-btn rounded raised&#x22;&#x3E;Follow&#x3C;/a&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="section section-feature-grey">
        <div class="container">
            <div id="click-dropdowns" class="section-title-wrapper has-text-centered">
                <div class="bg-number">2</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Click dropdowns
                </h2>
            </div>

            <!-- Basic click dropdowns -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Basic examples</h4>
                        <p class="no-padding">You can have more control on dropdowns by using click dropdown. In this case the trigger has to be clicked for the dropdown to show up. Bulkit click dropdowns are based on <a href="https://github.com/soundasleep/jquery-dropdown" target="_blank">Jquery Dropdown</a>. Click dropdowns are handled with the <code>data-jq-dropdown="#myDropdownId"</code>, pointing to a separate dropdown <code>div</code> elsewhere on the page. Click dropdowns can have a small arrow if you add the class <code>.jq-dropdown-tip</code> to the dropdown parent container.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-4 has-text-centered">
                    <button class="button" data-jq-dropdown="#with-tip">Dropdown</button>
                </div>
                <div class="column is-4 has-text-centered">
                    <button class="button btn-align accent-btn btn-outlined raised" data-jq-dropdown="#without-tip">Dropdown</button>
                </div>
                <div class="column is-4 has-text-centered">
                    <button class="button btn-align accent-btn btn-outlined raised" data-jq-dropdown="#without-tip">Dropdown</button>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-dropdowns.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;!-- Dropdown trigger --&#x3E;
        &#x3C;button class=&#x22;button&#x22; data-jq-dropdown=&#x22;#my-dropdown&#x22;&#x3E;Dropdown&#x3C;/button&#x3E;
        &#x3C;!-- /Dropdown trigger --&#x3E;
        
        &#x3C;!-- Dropdown --&#x3E;
        &#x3C;div id=&#x22;my-dropdown&#x22; class=&#x22;jq-dropdown jq-dropdown-tip&#x22;&#x3E;
            &#x3C;ul class=&#x22;jq-dropdown-menu&#x22;&#x3E;
                &#x3C;li&#x3E;&#x3C;a href=&#x22;#&#x22;&#x3E;Account&#x3C;/a&#x3E;&#x3C;/li&#x3E;
                &#x3C;li&#x3E;&#x3C;a href=&#x22;#&#x22;&#x3E;Tasks&#x3C;/a&#x3E;&#x3C;/li&#x3E;
                &#x3C;li&#x3E;&#x3C;a href=&#x22;#&#x22;&#x3E;Settings&#x3C;/a&#x3E;&#x3C;/li&#x3E;
                &#x3C;li class=&#x22;jq-dropdown-divider&#x22;&#x3E;&#x3C;/li&#x3E;
                &#x3C;li&#x3E;&#x3C;a href=&#x22;#&#x22;&#x3E;Log out&#x3C;/a&#x3E;&#x3C;/li&#x3E;
            &#x3C;/ul&#x3E;
        &#x3C;/div&#x3E;
        &#x3C;!-- /Dropdown --&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Click with icons -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>With icons</h4>
                        <p class="no-padding">Click dropdowns menu items can have icons as well. Just add the desired icons inside the menu items elements.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-3 is-offset-3 has-text-centered">
                    <button class="button" data-jq-dropdown="#with-icons-tip">Dropdown</button>
                </div>
                <div class="column is-3 has-text-centered">
                    <button class="button btn-align accent-btn btn-outlined raised" data-jq-dropdown="#with-icons-notip">Dropdown</button>
                </div>
            </div>

            <!-- Click with scroll -->
            <div class="columns is-vcentered mt-80">
                <div class="column is-6 is-offset-3">
                    <div class="info-block pb-20">
                        <h4>Scroll dropdown</h4>
                        <p class="no-padding">If you need to display a very long content inside your dropdown, you can use a scroll dropdown. To make your dropdown scroll automatically when there is enough content, add the <code>.jq-dropdown-scroll</code> class to the dropdown parent element.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3 has-text-centered">
                    <button class="button btn-align accent-btn btn-outlined raised" data-jq-dropdown="#with-scroll">Dropdown</button>
                </div>
            </div>

            <!-- With checkboxes and radio -->
            <div class="columns is-vcentered mt-80">
                <div class="column is-6 is-offset-3">
                    <div class="info-block pb-20">
                        <h4>Checkboxes and radios</h4>
                        <p class="no-padding">You can easily add controls inside click dropdowns. You can add either checkboxes or radio buttons. Check the code to see a live example.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3 has-text-centered">
                    <button class="button btn-align accent-btn btn-outlined raised" data-jq-dropdown="#with-checkboxes-radios">Dropdown</button>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-dropdowns.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;!-- Dropdown trigger --&#x3E;
        &#x3C;button class=&#x22;button btn-align accent-btn btn-outlined raised&#x22; data-jq-dropdown=&#x22;#with-checkboxes-radios&#x22;&#x3E; Dropdown&#x3C;/button&#x3E;
        &#x3C;!-- /Dropdown trigger --&#x3E;
        
        &#x3C;!-- Dropdown --&#x3E;
        &#x3C;div id=&#x22;with-checkboxes-radios&#x22; class=&#x22;jq-dropdown&#x22;&#x3E;
            &#x3C;ul class=&#x22;jq-dropdown-menu&#x22;&#x3E;
                &#x3C;li&#x3E;
                    &#x3C;label class=&#x22;checkbox-wrap is-small&#x22;&#x3E;
                        &#x3C;input id=&#x22;check1&#x22; type=&#x22;checkbox&#x22; class=&#x22;b-checkbox&#x22; onchange=&#x22;console.log(&#x27;changed&#x27;);&#x22;&#x3E;
                        &#x3C;span&#x3E;&#x3C;/span&#x3E;
                        Pro features
                    &#x3C;/label&#x3E;
                &#x3C;/li&#x3E;
                &#x3C;li&#x3E;
                    &#x3C;label class=&#x22;checkbox-wrap is-small&#x22;&#x3E;
                        &#x3C;input id=&#x22;check2&#x22; type=&#x22;checkbox&#x22; class=&#x22;b-checkbox&#x22; onchange=&#x22;console.log(&#x27;changed&#x27;);&#x22;&#x3E;
                        &#x3C;span&#x3E;&#x3C;/span&#x3E;
                        Auto backup
                    &#x3C;/label&#x3E;
                &#x3C;/li&#x3E;
                &#x3C;li&#x3E;
                    &#x3C;label class=&#x22;checkbox-wrap is-small&#x22;&#x3E;
                        &#x3C;input id=&#x22;check3&#x22; type=&#x22;checkbox&#x22; class=&#x22;b-checkbox&#x22; onchange=&#x22;console.log(&#x27;changed&#x27;);&#x22;&#x3E;
                        &#x3C;span&#x3E;&#x3C;/span&#x3E;
                        Dedicated support
                    &#x3C;/label&#x3E;
                &#x3C;/li&#x3E;
                &#x3C;li class=&#x22;jq-dropdown-divider&#x22;&#x3E;&#x3C;/li&#x3E;
                &#x3C;li&#x3E;
                    &#x3C;label class=&#x22;radio-wrap is-small is-primary&#x22;&#x3E;
                        &#x3C;input type=&#x22;radio&#x22; class=&#x22;b-radio&#x22; name=&#x22;radio-group&#x22;&#x3E;
                        &#x3C;span&#x3E;&#x3C;/span&#x3E;
                        Starter plan
                    &#x3C;/label&#x3E;
                &#x3C;/li&#x3E;
                &#x3C;li&#x3E;
                    &#x3C;label class=&#x22;radio-wrap is-small is-secondary&#x22;&#x3E;
                        &#x3C;input type=&#x22;radio&#x22; class=&#x22;b-radio&#x22; name=&#x22;radio-group&#x22;&#x3E;
                        &#x3C;span&#x3E;&#x3C;/span&#x3E;
                        Pro plan
                    &#x3C;/label&#x3E;
                &#x3C;/li&#x3E;
                &#x3C;li&#x3E;
                    &#x3C;label class=&#x22;radio-wrap is-small is-accent&#x22;&#x3E;
                        &#x3C;input type=&#x22;radio&#x22; class=&#x22;b-radio&#x22; name=&#x22;radio-group&#x22;&#x3E;
                        &#x3C;span&#x3E;&#x3C;/span&#x3E;
                        Business plan
                    &#x3C;/label&#x3E;
                &#x3C;/li&#x3E;
            &#x3C;/ul&#x3E;
        &#x3C;/div&#x3E;
        &#x3C;!-- /Dropdown --&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- hover colors -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Hover colors</h4>
                        <p class="no-padding">Click dropdowns menu items can have different colors on hover. Available classes are <code>.is-primary</code>, <code>.is-secondary</code> and <code>.is-accent</code>. Add one of these classes to the <code>.jq-dropdown</code> container.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-3 has-text-centered">
                    <button class="button" data-jq-dropdown="#with-icons-notip">Default</button>
                </div>
                <div class="column is-3 has-text-centered">
                    <button class="button btn-align primary-btn btn-outlined raised" data-jq-dropdown="#primary">Primary</button>
                </div>
                <div class="column is-3 has-text-centered">
                    <button class="button btn-align secondary-btn btn-outlined raised" data-jq-dropdown="#secondary">Secondary</button>
                </div>
                <div class="column is-3 has-text-centered">
                    <button class="button btn-align accent-btn btn-outlined raised" data-jq-dropdown="#accent">Accent</button>
                </div>
            </div>
        </div>
    </div>

    <div class="section">
        <div class="container">
            <div id="drop-panels" class="section-title-wrapper has-text-centered">
                <div class="bg-number">3</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Dropdown panels
                </h2>
            </div>

            <!-- Basic panel -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Basic example</h4>
                        <p class="no-padding">A variant of Jquery dropdowns are panels. Panels can contain regular html markup and serve many purposes. All jquery dropdown options also apply to panels.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-3 is-offset-3 has-text-centered">
                    <button class="button btn-align accent-btn btn-outlined raised" data-jq-dropdown="#tip-panel">Drop panel</button>
                </div>
                <div class="column is-3 has-text-centered">
                    <button class="button btn-align accent-btn btn-outlined raised" data-jq-dropdown="#notip-panel">Drop panel</button>
                </div>
            </div>

            <div class="columns is-vcentered mt-40">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-dropdowns.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;!-- Panel trigger --&#x3E;
        &#x3C;button class=&#x22;button btn-align accent-btn btn-outlined raised&#x22; data-jq-dropdown=&#x22;#tip-panel&#x22;&#x3E; Drop panel&#x3C;/button&#x3E;
        &#x3C;!-- /Panel trigger --&#x3E;
        
        &#x3C;!-- Panel --&#x3E;
        &#x3C;div id=&#x22;tip-panel&#x22; class=&#x22;jq-dropdown jq-dropdown-tip has-icons&#x22;&#x3E;
            &#x3C;div class=&#x22;jq-dropdown-panel&#x22;&#x3E;
                Here is some &#x3C;code&#x3E;HTML&#x3C;/code&#x3E; inside of a jq-dropdown. You can put pretty much
                anything inside of a jq-dropdown, so have fun with it!
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
        &#x3C;!-- /Panel --&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Notifications panel -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <span class="tag is-secondary">Reworked 3.0.0</span>
                        <h4>Notifications panel</h4>
                        <p class="no-padding">Here is an example of what you can do with a dropdown panel. this panel shows a user notifications panel, like you can find in many apps. Notification items rely on a modified Bulma <a href="http://bulma.io/documentation/layout/media-object/" target="_blank">Media object</a>.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3 has-text-centered">
                    <button class="button btn-align success-btn btn-outlined raised" data-jq-dropdown="#notifications-panel">Notifications</button>
                </div>
            </div>

            <div class="columns is-vcentered mt-40">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-dropdowns.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;!-- Panel trigger --&#x3E;
        &#x3C;button class=&#x22;button btn-align success-btn btn-outlined raised&#x22; data-jq-dropdown=&#x22;#notifications-panel&#x22;&#x3E; Notifications&#x3C;/button&#x3E;
        &#x3C;!-- /Panel trigger --&#x3E;
        
        &#x3C;!-- Panel --&#x3E;
        &#x3C;div id=&#x22;notifications-panel&#x22; class=&#x22;jq-dropdown jq-dropdown-tip has-icons&#x22;&#x3E;
            &#x3C;div class=&#x22;jq-dropdown-panel&#x22;&#x3E;
                &#x3C;article class=&#x22;media media-notification&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;p class=&#x22;image is-32x32&#x22;&#x3E;
                            &#x3C;img class=&#x22;img-circle&#x22; src=&#x22;assets/img/avatars/terry.jpg&#x22;&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span class=&#x22;notification-title&#x22;&#x3E;Terry Daniels&#x3C;/span&#x3E; &#x3C;small class=&#x22;timestamp pull-right&#x22;&#x3E;31 minutes ago&#x3C;/small&#x3E;
                                &#x3C;br&#x3E;
                                &#x3C;span class=&#x22;notification-text&#x22;&#x3E;Added a document to the project library.&#x3C;/span&#x3E;
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
                &#x3C;article class=&#x22;media media-notification&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;p class=&#x22;image is-32x32&#x22;&#x3E;
                            &#x3C;img class=&#x22;img-circle&#x22; src=&#x22;assets/img/avatars/carolin.png&#x22;&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span class=&#x22;notification-title&#x22;&#x3E;Marjory Cambell&#x3C;/span&#x3E; &#x3C;small class=&#x22;timestamp pull-right&#x22;&#x3E;2 hours ago&#x3C;/small&#x3E;
                                &#x3C;br&#x3E;
                                &#x3C;span class=&#x22;notification-text&#x22;&#x3E;Commented one of your tasks.&#x3C;/span&#x3E;
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
                &#x3C;article class=&#x22;media media-notification&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;p class=&#x22;image is-32x32&#x22;&#x3E;
                            &#x3C;img class=&#x22;img-circle&#x22; src=&#x22;assets/img/avatars/henry.jpg&#x22;&#x3E;
                        &#x3C;/p&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span class=&#x22;notification-title&#x22;&#x3E;Henry Rogers&#x3C;/span&#x3E; &#x3C;small class=&#x22;timestamp pull-right&#x22;&#x3E;3 hours ago&#x3C;/small&#x3E;
                                &#x3C;br&#x3E;
                                &#x3C;span class=&#x22;notification-text&#x22;&#x3E;Added new tasks to project #122.&#x3C;/span&#x3E;
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
                &#x3C;div class=&#x22;see-all has-text-centered&#x22;&#x3E;
                    &#x3C;a href=&#x22;#&#x22;&#x3E;All notifications&#x3C;/a&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
        &#x3C;!-- /Panel --&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Bookmarks panel -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <span class="tag is-secondary">Reworked 3.0.0</span>
                        <h4>Bookmarks panel</h4>
                        <p class="no-padding">Here is another example of dropdown panel, this one is used to display previsously added bookmarks.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3 has-text-centered">
                    <button class="button btn-align warning-btn btn-outlined raised" data-jq-dropdown="#bookmarks-panel">Bookmarks</button>
                </div>
            </div>

            <div class="columns is-vcentered mt-40 pb-60">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-dropdowns.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;!-- Panel trigger --&#x3E;
        &#x3C;button class=&#x22;button btn-align warning-btn btn-outlined raised&#x22; data-jq-dropdown=&#x22;#bookmarks-panel&#x22;&#x3E; Bookmarks&#x3C;/button&#x3E;
        &#x3C;!-- /Panel trigger --&#x3E;
        
        &#x3C;!-- Panel --&#x3E;
        &#x3C;div id=&#x22;bookmarks-panel&#x22; class=&#x22;jq-dropdown jq-dropdown-tip has-icons&#x22;&#x3E;
            &#x3C;div class=&#x22;jq-dropdown-panel&#x22;&#x3E;
                &#x3C;article class=&#x22;media icon-media&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;i class=&#x22;im im-icon-File-Word&#x22;&#x3E;&#x3C;/i&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span class=&#x22;notification-title&#x22;&#x3E;Project_budget.docx&#x3C;/span&#x3E; &#x3C;small class=&#x22;timestamp pull-right&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-lock&#x22;&#x3E;&#x3C;/i&#x3E; Locked&#x3C;/small&#x3E;
                                &#x3C;br&#x3E;
                                &#x3C;span class=&#x22;notification-text&#x22;&#x3E;Word document&#x3C;/span&#x3E;
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
                &#x3C;article class=&#x22;media icon-media&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;i class=&#x22;im im-icon-File-Excel&#x22;&#x3E;&#x3C;/i&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span class=&#x22;notification-title&#x22;&#x3E;Accounts_stakeholders.xlsx&#x3C;/span&#x3E; &#x3C;small class=&#x22;timestamp pull-right&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-lock-open&#x22;&#x3E;&#x3C;/i&#x3E; Unlocked&#x3C;/small&#x3E;
                                &#x3C;br&#x3E;
                                &#x3C;span class=&#x22;notification-text&#x22;&#x3E;Excel document&#x3C;/span&#x3E;
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
                &#x3C;article class=&#x22;media icon-media&#x22;&#x3E;
                    &#x3C;figure class=&#x22;media-left&#x22;&#x3E;
                        &#x3C;i class=&#x22;im im-icon-File-JPG&#x22;&#x3E;&#x3C;/i&#x3E;
                    &#x3C;/figure&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;div class=&#x22;content&#x22;&#x3E;
                            &#x3C;p&#x3E;
                                &#x3C;span class=&#x22;notification-title&#x22;&#x3E;Train_receipts.jpg&#x3C;/span&#x3E; &#x3C;small class=&#x22;timestamp pull-right&#x22;&#x3E;&#x3C;i class=&#x22;sl sl-icon-lock-open&#x22;&#x3E;&#x3C;/i&#x3E; Unlocked&#x3C;/small&#x3E;
                                &#x3C;br&#x3E;
                                &#x3C;span class=&#x22;notification-text&#x22;&#x3E;Image document&#x3C;/span&#x3E;
                            &#x3C;/p&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/article&#x3E;
                &#x3C;div class=&#x22;see-all has-text-centered&#x22;&#x3E;
                    &#x3C;a href=&#x22;#&#x22;&#x3E;All bookmarks&#x3C;/a&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
        &#x3C;!-- /Panel --&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>

    <!-- Dark footer -->
    <footer id="dark-footer" class="footer footer-dark">
        <div class="container">
            <div class="columns">
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Product</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-dropdowns.php#">Discover features</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">CMS integration</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">Customers</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">Weekly sessions</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">Free trials and v1</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">What's next ?</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Company</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-dropdowns.php#">About us</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">About security</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">User guide</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">Terms of service</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">Terms of website use</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">Privacy policy</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Learning</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-dropdowns.php#">Resources</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">Blog</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">FAQ</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">API documentation</a></li>
                            <li><a href="_components-basicui-dropdowns.php#">Admin guide</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-logo">
                            <img class="switcher-logo-w" src="assets/img/logos/logo/bulkit-core-w.svg" alt="">
                        </div>
                        <div class="footer-header">
                            <nav class="level is-mobile">
                                <div class="level-left level-social">
                                    <a href="_components-basicui-dropdowns.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-facebook"></i></span>
                                    </a>
                                    <a href="_components-basicui-dropdowns.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-twitter"></i></span>
                                    </a>
                                    <a href="_components-basicui-dropdowns.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-linkedin"></i></span>
                                    </a>
                                    <a href="_components-basicui-dropdowns.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-dribbble"></i></span>
                                    </a>
                                    <a href="_components-basicui-dropdowns.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-github"></i></span>
                                    </a>
                                </div>
                            </nav>
                        </div>
                        <div class="copyright">
                            <span class="moto light-text">Designed and coded with <i class="fa fa-heart color-red"></i> by CSS Ninja.</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </footer>
    <!-- /Dark footer -->
    <!-- DROPDOWNS -->

    <!-- With tip -->
    <div id="with-tip" class="jq-dropdown jq-dropdown-tip">
        <ul class="jq-dropdown-menu">
            <li><a href="_components-basicui-dropdowns.php#">Account</a></li>
            <li><a href="_components-basicui-dropdowns.php#">Tasks</a></li>
            <li><a href="_components-basicui-dropdowns.php#">Settings</a></li>
            <li class="jq-dropdown-divider"></li>
            <li><a href="_components-basicui-dropdowns.php#">Log out</a></li>
        </ul>
    </div>

    <!-- Without tip -->
    <div id="without-tip" class="jq-dropdown">
        <ul class="jq-dropdown-menu">
            <li><a href="_components-basicui-dropdowns.php#">Account</a></li>
            <li><a href="_components-basicui-dropdowns.php#">Tasks</a></li>
            <li><a href="_components-basicui-dropdowns.php#">Settings</a></li>
            <li class="jq-dropdown-divider"></li>
            <li><a href="_components-basicui-dropdowns.php#">Log out</a></li>
        </ul>
    </div>

    <!-- With icons tip -->
    <div id="with-icons-tip" class="jq-dropdown jq-dropdown-tip">
        <ul class="jq-dropdown-menu">
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-user mr-10"></i> Account</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-check mr-10"></i> Tasks</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-settings mr-10"></i> Settings</a></li>
            <li class="jq-dropdown-divider"></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-logout mr-10"></i> Log out</a></li>
        </ul>
    </div>

    <!-- With icons no tip -->
    <div id="with-icons-notip" class="jq-dropdown">
        <ul class="jq-dropdown-menu">
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-user mr-10"></i> Account</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-check mr-10"></i> Tasks</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-settings mr-10"></i> Settings</a></li>
            <li class="jq-dropdown-divider"></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-logout mr-10"></i> Log out</a></li>
        </ul>
    </div>

    <!-- With scroll -->
    <div id="with-scroll" class="jq-dropdown jq-dropdown-scroll">
        <ul class="jq-dropdown-menu">
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-pencil mr-10"></i> Edit document</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-cloud-upload mr-10"></i> Ulpoad new version</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-clock mr-10"></i> Manage versions</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-check mr-10"></i> Start a workflow</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-user mr-10"></i> Assign user</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-star mr-10"></i> Add to bookmarks</a></li>
            <li class="jq-dropdown-divider"></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-trash mr-10"></i> Delete</a></li>
        </ul>
    </div>

    <!-- With checkboxes radios -->
    <div id="with-checkboxes-radios" class="jq-dropdown">
        <ul class="jq-dropdown-menu">
            <li>
                <label class="checkbox-wrap is-small">
                    <input id="check1" type="checkbox" class="b-checkbox" onchange="console.log('changed');">
                    <span></span>
                    Pro features
                </label>
            </li>
            <li>
                <label class="checkbox-wrap is-small">
                    <input id="check2" type="checkbox" class="b-checkbox" onchange="console.log('changed');">
                    <span></span>
                    Auto backup
                </label>
            </li>
            <li>
                <label class="checkbox-wrap is-small">
                    <input id="check3" type="checkbox" class="b-checkbox" onchange="console.log('changed');">
                    <span></span>
                    Dedicated support
                </label>
            </li>
            <li class="jq-dropdown-divider"></li>
            <li>
                <label class="radio-wrap is-small is-primary">
                    <input type="radio" class="b-radio" name="radio-group">
                    <span></span>
                    Starter plan
                </label>
            </li>
            <li>
                <label class="radio-wrap is-small is-secondary">
                    <input type="radio" class="b-radio" name="radio-group">
                    <span></span>
                    Pro plan
                </label>
            </li>
            <li>
                <label class="radio-wrap is-small is-accent">
                    <input type="radio" class="b-radio" name="radio-group">
                    <span></span>
                    Business plan
                </label>
            </li>
        </ul>
    </div>

    <!-- Primary click -->
    <div id="primary" class="jq-dropdown is-primary">
        <ul class="jq-dropdown-menu">
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-user mr-10"></i> Account</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-check mr-10"></i> Tasks</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-settings mr-10"></i> Settings</a></li>
            <li class="jq-dropdown-divider"></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-logout mr-10"></i> Log out</a></li>
        </ul>
    </div>

    <!-- Secondary click -->
    <div id="secondary" class="jq-dropdown is-secondary">
        <ul class="jq-dropdown-menu">
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-user mr-10"></i> Account</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-check mr-10"></i> Tasks</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-settings mr-10"></i> Settings</a></li>
            <li class="jq-dropdown-divider"></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-logout mr-10"></i> Log out</a></li>
        </ul>
    </div>

    <!-- Accent click -->
    <div id="accent" class="jq-dropdown is-accent">
        <ul class="jq-dropdown-menu">
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-user mr-10"></i> Account</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-check mr-10"></i> Tasks</a></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-settings mr-10"></i> Settings</a></li>
            <li class="jq-dropdown-divider"></li>
            <li><a href="_components-basicui-dropdowns.php#"><i class="sl sl-icon-logout mr-10"></i> Log out</a></li>
        </ul>
    </div>

    <!-- Tip panel -->
    <div id="tip-panel" class="jq-dropdown jq-dropdown-tip has-icons">
        <div class="jq-dropdown-panel">
            Here is some <code>HTML</code> inside of a jq-dropdown. You can put pretty much
            anything inside of a jq-dropdown, so have fun with it!
        </div>
    </div>

    <!-- noTip panel -->
    <div id="notip-panel" class="jq-dropdown has-icons">
        <div class="jq-dropdown-panel">
            Here is some <code>HTML</code> inside of a jq-dropdown. You can put pretty much
            anything inside of a jq-dropdown, so have fun with it!
        </div>
    </div>

    <!-- Notifications panel -->
    <div id="notifications-panel" class="jq-dropdown jq-dropdown-tip has-icons">
        <div class="jq-dropdown-panel">
            <div class="media media-notification">
                <figure class="media-left">
                    <p class="image is-32x32">
                        <img class="img-circle" src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/terry.jpg">
                    </p>
                </figure>
                <div class="media-content">
                    <div class="content">
                        <p>
                            <span class="notification-title is-block">Terry Daniels</span>
                            <span class="notification-text is-block">Added a document to the project library.</span>
                            <small class="timestamp is-block">31 minutes ago</small>
                        </p>
                    </div>
                </div>
            </div>
            <div class="media media-notification">
                <figure class="media-left">
                    <p class="image is-32x32">
                        <img class="img-circle" src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/carolin.png">
                    </p>
                </figure>
                <div class="media-content">
                    <div class="content">
                        <p>
                            <span class="notification-title is-block">Marjory Cambell</span>
                            <span class="notification-text is-block">Commented one of your tasks.</span>
                            <small class="timestamp is-block">2 hours ago</small>
                        </p>
                    </div>
                </div>
            </div>
            <div class="media media-notification">
                <figure class="media-left">
                    <p class="image is-32x32">
                        <img class="img-circle" src="https://via.placeholder.com/250x250" alt="" data-v1-src="assets/img/avatars/henry.jpg">
                    </p>
                </figure>
                <div class="media-content">
                    <div class="content">
                        <p>
                            <span class="notification-title is-block">Henry Rogers</span>
                            <span class="notification-text is-block">Added new tasks to project #122.</span>
                            <small class="timestamp is-block">3 hours ago</small>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Bookmarks panel -->
    <div id="bookmarks-panel" class="jq-dropdown jq-dropdown-tip has-icons">
        <div class="jq-dropdown-panel">
            <div class="media icon-media">
                <figure class="media-left">
                    <i class="im im-icon-Add-File"></i>
                </figure>
                <div class="media-content">
                    <div class="content">
                        <p>
                            <span class="notification-title is-block">Project_budget.docx</span>
                            <span class="notification-text is-block">Word document</span>
                        </p>
                    </div>
                </div>
            </div>
            <div class="media icon-media">
                <figure class="media-left">
                    <i class="im im-icon-Add-File"></i>
                </figure>
                <div class="media-content">
                    <div class="content">
                        <p>
                            <span class="notification-title is-block">Accounts_stakeholders.xlsx</span>
                            <span class="notification-text is-block">Excel document</span>
                        </p>
                    </div>
                </div>
            </div>
            <div class="media icon-media">
                <figure class="media-left">
                    <i class="im im-icon-Add-File"></i>
                </figure>
                <div class="media-content">
                    <div class="content">
                        <p>
                            <span class="notification-title is-block">Train_receipts.jpg</span>
                            <span class="notification-text is-block">Image document</span>
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Side navigation -->
    <div class="side-navigation-menu">
        <!-- Categories menu -->
        <div class="category-menu-wrapper">
            <!-- Menu -->
            <ul class="categories">
                <li class="square-logo"><img src="assets/img/logos/square-white.svg" alt=""></li>
                <li class="category-link is-active" data-navigation-menu="v1-pages"><i class="sl sl-icon-layers"></i></li>
                <li class="category-link" data-navigation-menu="onepagers"><i class="sl sl-icon-docs"></i></li>
                <li class="category-link" data-navigation-menu="components"><i class="sl sl-icon-grid"></i></li>
            </ul>
            <!-- Menu -->

            <ul class="author">
                <li>
                    <!-- Theme author -->
                    <a href="https://cssninja.io" target="_blank">
                        <img class="main-menu-author" src="assets/img/logos/cssninja.svg" alt="">
                    </a>
                </li>
            </ul>
        </div>
        <!-- /Categories menu -->

        <!-- Navigation menu -->
        <div id="v1-pages" class="navigation-menu-wrapper animated preFadeInRight fadeInRight">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>V1 pages</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation Body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">weekend</span>Agency kit</a>
                    <ul>
                        <li><a class="is-submenu" href="agency-home.php">Homepage</a></li>
                        <li><a class="is-submenu" href="agency-about.php">About</a></li>
                        <li><a class="is-submenu" href="agency-portfolio.php">Portfolio</a></li>
                        <li><a class="is-submenu" href="agency-contact.php">Contact</a></li>
                        <li><a class="is-submenu" href="agency-blog.php">Blog</a></li>
                        <li><a class="is-submenu" href="agency-post-nosidebar.php">Post no sidebar</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">wb_incandescent</span>Startup kit</a>
                    <ul>
                        <li><a class="is-submenu" href="startup-landing.php">Homepage</a></li>
                        <li><a class="is-submenu" href="startup-about.php">About</a></li>
                        <li><a class="is-submenu" href="startup-product.php">Product</a></li>
                        <li><a class="is-submenu" href="startup-contact.php">Contact</a></li>
                        <li><a class="is-submenu" href="startup-login.php">Login</a></li>
                        <li><a class="is-submenu" href="startup-signup.php">Sign up</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">apps</span>Landing kit 1</a>
                    <ul>
                        <li><a class="is-submenu" href="kit1-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit1-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit1-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit1-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit1-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit1-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit1-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit1-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit1-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit1-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">timelapse</span>Landing kit 2</a>
                    <ul>
                        <li><a class="is-submenu" href="kit2-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit2-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit2-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit2-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit2-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit2-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit2-landing-7.php">Landing page v7</a></li>
                        <li><a class="is-submenu" href="kit2-landing-8.php">Landing page v8</a></li>
                        <li><a class="is-submenu" href="kit2-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit2-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit2-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit2-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">assistant</span>Landing kit 3</a>
                    <ul>
                        <li><a class="is-submenu" href="kit3-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit3-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit3-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit3-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit3-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit3-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">brightness_2</span>Landing kit 4</a>
                    <ul>
                        <li><a class="is-submenu" href="kit4-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit4-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit4-help.php">Help center</a></li>
                        <li><a class="is-submenu" href="kit4-help-category.php">Help category</a></li>
                        <li><a class="is-submenu" href="kit4-help-article.php">Help article</a></li>
                        <li><a class="is-submenu" href="kit4-signup.php">Sign Up</a></li>
                        <li><a class="is-submenu" href="kit4-login.php">Login</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">toys</span>Landing kit 5</a>
                    <ul>
                        <li><a class="is-submenu" href="kit5-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit5-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit5-pricing.php">Pricing Page</a></li>
                        <li><a class="is-submenu" href="kit5-login.php">Login / Signup</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">mouse</span>Landing kit 6</a>
                    <ul>
                        <li><a class="is-submenu" href="kit6-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit6-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit6-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">work</span>Landing kit
                        7</a>
                    <ul>
                        <li><a class="is-submenu" href="kit7-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit7-landing.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit7-authentication.php">Auth page</a></li>
                        <li><a class="is-submenu" href="kit7-pricing.php">Pricing page</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-dropdowns.php#">
                        <span class="material-icons">public</span>Landing kit 8</a>
                    <ul>
                        <li><a class="is-submenu" href="kit8-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit8-landing-1.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit8-landing-2.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit8-landing-3.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit8-landing-4.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit8-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit8-jobs.php">Jobs page</a></li>
                        <li><a class="is-submenu" href="kit8-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit8-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-dropdowns.php#">
                        <span class="material-icons">assessment</span>Landing kit 9</a>
                    <ul>
                        <li><a class="is-submenu" href="kit9-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit9-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit9-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit9-landing-4.php">Landing page v4</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-dropdowns.php#">
                        <span class="material-icons">account_balance</span>Landing kit 10</a>
                    <ul>
                        <li><a class="is-submenu" href="kit10-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit10-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit10-landing-3.php">Landing page v3</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-dropdowns.php#">
                        <span class="material-icons">shop</span>Landing kit 11</a>
                    <ul>
                        <li><a class="is-submenu" href="kit11-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit11-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit11-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit11-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit11-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit11-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit11-landing-7.php">Landing page v7</a></li>
                        <li><a class="is-submenu" href="kit11-landing-8.php">Landing page v8</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-dropdowns.php#">
                        <span class="material-icons">stars</span>Landing kit 12</a>
                    <ul>
                        <li><a class="is-submenu" href="kit12-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit12-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit12-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit12-landing-4.php">Landing page v4</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-dropdowns.php#">
                        <span class="material-icons">new_releases</span>Landing kit 13</a>
                    <ul>
                        <li><a class="is-submenu" href="kit13-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit13-landing-2.php">Landing page v2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-dropdowns.php#">
                        <span class="material-icons">code</span>Landing kit 14</a>
                    <ul>
                        <li><a class="is-submenu" href="kit14-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit14-landing-2.php">Landing page v2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-dropdowns.php#">
                        <span class="material-icons">settings</span>Landing kit 15</a>
                    <ul>
                        <li><a class="is-submenu" href="kit15-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit15-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit15-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit15-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit15-about.php">About page</a></li>
                        <li><a class="is-submenu" href="kit15-contact.php">Contact page</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- Navigation menu -->
        <div id="onepagers" class="navigation-menu-wrapper animated preFadeInRight fadeInRight is-hidden">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>Sub Pages</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-dropdowns.php#"><span class="material-icons">domain</span>Company</a>
                    <ul>
                        <li><a class="is-submenu" href="about-page-1.php">About v1</a></li>
                        <li><a class="is-submenu" href="about-page-2.php">About v2</a></li>
                        <li><a class="is-submenu" href="case-study-1.php">Case Study v1</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-dropdowns.php#"><span class="material-icons">call</span>Contact</a>
                    <ul>
                        <li><a class="is-submenu" href="contact-page-1.php">Contact v1</a></li>
                        <li><a class="is-submenu" href="contact-page-2.php">Contact v2</a></li>
                        <li><a class="is-submenu" href="contact-page-3.php">Contact v3</a></li>
                        <li><a class="is-submenu" href="contact-page-4.php">Contact v4</a></li>
                        <li><a class="is-submenu" href="contact-page-5.php">Contact v5</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-dropdowns.php#"><span class="material-icons">book</span>Blog</a>
                    <ul>
                        <li><a class="is-submenu" href="blog-posts-full.php">Posts List v1</a></li>
                        <li><a class="is-submenu" href="blog-posts-full-alt.php">Posts List v2</a></li>
                        <li><a class="is-submenu" href="blog-posts-side.php">Posts List v3</a></li>
                        <li><a class="is-submenu" href="blog-posts-side-alt.php">Posts List v4</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-full.php">Posts Grid v1</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-full-masonry.php">Posts Grid v2</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-side.php">Posts Grid v3</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-side-masonry.php">Posts Grid v4</a></li>
                        <li><a class="is-submenu" href="blog-single-full.php">Single Post V1</a></li>
                        <li><a class="is-submenu" href="blog-single-side.php">Single Post V2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link has-new" href="_components-basicui-dropdowns.php#"><span class="material-icons">highlight</span>Error Pages</a>
                    <ul>
                        <li><a class="is-submenu" href="error-1.php">Error v1</a></li>
                        <li><a class="is-submenu" href="error-2.php">Error v2</a></li>
                        <li><a class="is-submenu" href="error-3.php">Error v3</a></li>
                        <li><a class="is-submenu" href="error-4.php">Error v4</a></li>
                        <li><a class="is-submenu" href="error-5.php">Error v5</a></li>
                        <li><a class="is-submenu" href="error-6.php">Error v6</a></li>
                        <li><a class="is-submenu" href="error-7.php">Error v7</a></li>
                        <li><a class="is-submenu" href="error-8.php">Error v8</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- Navigation menu -->
        <div id="components" class="navigation-menu-wrapper animated preFadeInRight fadeInRight is-hidden">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>Components</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">view_quilt</span>Layout</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-layout-grid.php">Grid system</a></li>
                        <li><a class="is-submenu" href="_components-layout-video.php">Video background</a></li>
                        <li><a class="is-submenu" href="_components-layout-parallax.php">Parallax</a></li>
                        <li><a class="is-submenu" href="_components-layout-headers.php">Headers</a></li>
                        <li><a class="is-submenu" href="_components-layout-footers.php">Footers</a></li>
                        <li><a class="is-submenu" href="_components-layout-typography.php">Typography</a></li>
                        <li><a class="is-submenu" href="_components-layout-colors.php">Colors</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">subject</span>Navbars</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-layout-navbar-fade-light.php">Fade light</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-fade-dark.php">Fade dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-light.php">Static
                                light</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-dark.php">Static
                                dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-solid.php">Static
                                solid</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-double-dark.php">Double
                                dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-double-light.php">Double
                                light</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">view_stream</span>Sections</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-sections-features.php">Features</a></li>
                        <li><a class="is-submenu" href="_components-sections-pricing.php">Pricing</a></li>
                        <li><a class="is-submenu" href="_components-sections-team.php">Team</a></li>
                        <li><a class="is-submenu" href="_components-sections-testimonials.php">Testimonials</a></li>
                        <li><a class="is-submenu" href="_components-sections-clients.php">Clients</a></li>
                        <li><a class="is-submenu" href="_components-sections-counters.php">Counters</a></li>
                        <li><a class="is-submenu" href="_components-sections-carousel.php">Carousel</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">playlist_add_check</span>Basic
                        UI</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-basicui-cards.php">Cards</a></li>
                        <li><a class="is-submenu" href="_components-basicui-buttons.php">Buttons</a></li>
                        <li><a class="is-submenu" href="_components-basicui-dropdowns.php">Dropdowns</a></li>
                        <li><a class="is-submenu" href="_components-basicui-lists.php">Lists</a></li>
                        <li><a class="is-submenu" href="_components-basicui-modals.php">Modals</a></li>
                        <li><a class="is-submenu" href="_components-basicui-tabs.php">Tabs & pills</a></li>
                        <li><a class="is-submenu" href="_components-basicui-accordion.php">Accordions</a></li>
                        <li><a class="is-submenu" href="_components-basicui-badges.php">Badges & labels</a></li>
                        <li><a class="is-submenu" href="_components-basicui-popups.php">Popups</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">playlist_add</span>Advanced
                        UI</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-advancedui-tables.php">Tables</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-timeline.php">Timeline</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-boxes.php">Boxes</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-messages.php">Messages</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-megamenu.php">Megamenu</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-calendar.php">Calendar</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">check_box</span>Forms</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-forms-inputs.php">Inputs</a></li>
                        <li><a class="is-submenu" href="_components-forms-controls.php">Controls</a></li>
                        <li><a class="is-submenu" href="_components-forms-layouts.php">Form layouts</a></li>
                        <li><a class="is-submenu" href="_components-forms-steps.php">Step forms</a></li>
                        <li><a class="is-submenu" href="_components-forms-uploader.php">Uploader</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-dropdowns.php#"><span class="material-icons">adjust</span>Icons</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-icons-im.php">Icons Mind</a></li>
                        <li><a class="is-submenu" href="_components-icons-sl.php">Simple Line Icons</a></li>
                        <li><a class="is-submenu" href="_components-icons-fa.php">Font Awesome</a></li>
                        <li><a class="is-submenu" href="https://material.io/icons/" target="_blank">Material Icons</a></li>
                        <li><a class="is-submenu" href="_components-extensions-iconpicker.php">Icon Picker</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- /Navigation menu -->
    </div>
    <!-- /Side navigation -->
    <!-- Back To Top Button -->
    <div id="backtotop"><a href="_components-basicui-dropdowns.php#"></a></div> <!-- Concatenated jQuery and plugins -->
    <script src="assets/js/app.js"></script>

    <!-- Bulkit js -->
    <script src="assets/js/functions.js"></script>
    <script src="assets/js/main.js"></script>
</body>

</html>