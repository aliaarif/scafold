<!DOCTYPE html>
<html lang="en">

<head>
    <!-- Required Meta Tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta http-equiv="x-ua-compatible" content="ie=edge">

    <title> Bulkit :: Cards</title>
    <link rel="icon" type="image/png" href="assets/img/favicon.png" />

    <!--Core CSS -->
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;500;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="assets/css/bulma.css">
    <link rel="stylesheet" href="assets/css/app.css">
    <link rel="stylesheet" href="assets/css/core.css">
</head>

<body id="component-page">
    <div class="pageloader"></div>
    <div class="infraloader is-active"></div>
    <!-- Hero and nav -->
    <div class="hero is-relative is-theme-primary">

        <nav class="navbar navbar-wrapper navbar-light is-transparent is-static">
            <div class="container">
                <!-- Brand -->
                <div class="navbar-brand">
                    <a class="navbar-item" href="index.php">
                        <img src="assets/img/logos/bulkit-white.svg" alt="">
                    </a>

                    <!-- Sidebar Trigger -->
                    <a id="navigation-trigger" class="navbar-item hamburger-btn" href="javascript:void(0);">
                        <span class="menu-toggle">
                                <span class="icon-box-toggle">
                                    <span class="rotate">
                                        <i class="icon-line-top"></i>
                                        <i class="icon-line-center"></i>
                                        <i class="icon-line-bottom"></i>
                                    </span>
                        </span>
                        </span>
                    </a>

                    <!-- Responsive toggle -->
                    <div class="custom-burger" data-target="">
                        <a id="" class="responsive-btn" href="javascript:void(0);">
                            <span class="menu-toggle">
                                    <span class="icon-box-toggle">
                                        <span class="rotate">
                                            <i class="icon-line-top"></i>
                                            <i class="icon-line-center"></i>
                                            <i class="icon-line-bottom"></i>
                                        </span>
                            </span>
                            </span>
                        </a>
                    </div>
                    <!-- /Responsive toggle -->
                </div>

                <!-- Navbar menu -->
                <div class="navbar-menu">
                    <!-- Navbar Start -->
                    <div class="navbar-start">
                        <!-- Dropdown -->
                        <div class="navbar-item has-dropdown is-hoverable is-mega">
                            <div class="navbar-link">
                                Menu
                            </div>
                            <div class="navbar-dropdown is-megamenu animated prefadeInUp fadeInUp" data-style="width: 18rem;">
                                <div class="mega-inner">
                                    <div class="container">
                                        <div class="columns">
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Landing Kits</h2>
                                                <div class="scrollable-container" data-simplebar>
                                                    <a class="navbar-item is-flex" href="kit1-landing.php">
                                                        <i class="im im-icon-Navigation-RightWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 1</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit2-landing.php">
                                                        <i class="im im-icon-Split-Horizontal"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 2</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit3-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 3</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit4-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 4</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit5-landing.php">
                                                        <i class="im im-icon-Love-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 5</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit6-landing.php">
                                                        <i class="im im-icon-Two-Windows"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 6</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit7-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 7</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit5-landing.php">
                                                        <i class="im im-icon-Love-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 5</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit6-landing.php">
                                                        <i class="im im-icon-Two-Windows"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 6</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit7-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 7</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit8-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 8</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit9-landing.php">
                                                        <i class="im im-icon-Split-FourSquareWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 9</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit10-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 10</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit11-landing.php">
                                                        <i class="im im-icon-Favorite-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 11</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit12-landing.php">
                                                        <i class="im im-icon-Split-Vertical"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 12</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit13-landing.php">
                                                        <i class="im im-icon-Split-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 13</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit14-landing.php">
                                                        <i class="im im-icon-Sidebar-Window"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 14</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="kit15-landing.php">
                                                        <i class="im im-icon-Split-FourSquareWindow"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">KIT 15</small>
                                                            </p>
                                                            <p>Landing page</p>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Utility Pages</h2>
                                                <div class="scrollable-container" data-simplebar="">
                                                    <a class="navbar-item is-flex" href="about-page-1.php">
                                                        <i class="im im-icon-Building"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">ABOUT</small>
                                                            </p>
                                                            <p>About v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="about-page-2.php">
                                                        <i class="im im-icon-Building"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">ABOUT</small>
                                                            </p>
                                                            <p>About v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="about-page-2.php">
                                                        <i class="im im-icon-Structure"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CASE STUDY</small>
                                                            </p>
                                                            <p>Case Study v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-1.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-2.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-3.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-4.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="contact-page-5.php">
                                                        <i class="im im-icon-Mail"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">CONTACT</small>
                                                            </p>
                                                            <p>Contact v5</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-full-alt.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-posts-side-alt.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts List v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-full-masonry.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v2</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v3</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-grid-side-masonry.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Posts Grid v4</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-single-full.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Single Post v1</p>
                                                        </div>
                                                    </a>
                                                    <a class="navbar-item is-flex" href="blog-single-side.php">
                                                        <i class="im im-icon-Newspaper"></i>
                                                        <div class="navbar-content">
                                                            <p>
                                                                <small class="color-primary">BLOG</small>
                                                            </p>
                                                            <p>Single Post v2</p>
                                                        </div>
                                                    </a>
                                                </div>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">Themed Kits</h2>
                                                <a class="navbar-item is-flex" href="agency-home.php">
                                                    <i class="im im-icon-Post-Office"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">AGENCY</small>
                                                        </p>
                                                        <p>Agency landing</p>
                                                    </div>
                                                </a>
                                                <a class="navbar-item is-flex" href="startup-home.php">
                                                    <i class="im im-icon-Factory"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">STARTUP</small>
                                                        </p>
                                                        <p>Startup landing</p>
                                                    </div>
                                                </a>
                                                <a class="navbar-item is-flex" href="error-1.php">
                                                    <i class="im im-icon-Coffin"></i>
                                                    <div class="navbar-content">
                                                        <p>
                                                            <small class="color-primary">ERROR KIT</small>
                                                        </p>
                                                        <p>Error Pages</p>
                                                    </div>
                                                </a>
                                            </div>
                                            <div class="column">
                                                <h2 class="title is-6 is-mega-menu-title">More goodies</h2>
                                                <p class="is-cta">As a Bulkit customer, you get some free goodies on Css Ninja.</p>
                                                <p class="is-cta">
                                                    <a href="https://cssninja.io" target="_blank" class="button btn-align mega-btn primary-btn">Learn more</a>
                                                </p>
                                            </div>
                                        </div>
                                    </div>

                                    <hr class="navbar-divider">
                                    <div class="navbar-item is-footer is-hidden-mobile">
                                        <div class="navbar-content">
                                            <div class="level is-mobile">
                                                <div class="level-left">
                                                    <div class="level-item">
                                                        <strong>Follow us</strong>
                                                    </div>
                                                </div>
                                                <div class="level-right">
                                                    <div class="level-item">
                                                        <a class="social-link" href="_components-basicui-cards.php#"><i class="fa fa-twitter"></i></a>
                                                        <a class="social-link" href="_components-basicui-cards.php#"><i class="fa fa-facebook"></i></a>
                                                        <a class="social-link" href="_components-basicui-cards.php#"><i class="fa fa-behance"></i></a>
                                                        <a class="social-link" href="_components-basicui-cards.php#"><i class="fa fa-github"></i></a>
                                                        <a class="social-link" href="_components-basicui-cards.php#"><i class="fa fa-tumblr"></i></a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="menu-image is-hidden-mobile">
                                        <img src="assets/img/graphics/legacy/megamenu-core.svg" data-base-url="assets/img/graphics/legacy/megamenu" data-extension=".svg" alt="">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <!-- Mobile dropdown -->
                        <!--div class="mobile-drop is-hidden-desktop">
                                <div class="mobile-drop-trigger">
                                    V1 <i class="material-icons">keyboard_arrow_right</i>
                                </div>
                                <div class="child-menu">
                                    <ul>
                                        <li>
                                            <a href="/agency-home.php">Agency kit</a>
                                        </li>
                                        <li>
                                            <a href="/startup-landing.php">Startup kit</a>
                                        </li>
                                        <li>
                                            <a href="/kit1-landing.php">Landing kit 1</a>
                                        </li>
                                        <li>
                                            <a href="/kit2-landing.php">Landing kit 2</a>
                                        </li>
                                        <li>
                                            <a href="/kit3-landing.php">Landing kit 3</a>
                                        </li>
                                        <li>
                                            <a href="/kit4-landing.php">Landing kit 4</a>
                                        </li>
                                        <li>
                                            <a href="/kit5-landing.php">Landing kit 5</a>
                                        </li>
                                        <li>
                                            <a href="/kit6-landing.php">Landing kit 6</a>
                                        </li>
                                        <li>
                                            <a href="/kit7-landing.php">Landing kit 7</a>
                                        </li>
                                        <li>
                                            <a href="/kit8-landing.php">Landing kit 8</a>
                                        </li>
                                        <li>
                                            <a href="/kit9-landing.php">Landing kit 9</a>
                                        </li>
                                        <li>
                                            <a href="/kit10-landing.php">Landing kit 10</a>
                                        </li>
                                        <li>
                                            <a href="/kit11-landing.php">Landing kit 11</a>
                                        </li>
                                    </ul>
                                </div>                </div-->
                        <!-- Navbar item -->
                        <a class="navbar-item is-slide" href="https://docs.bulkit.cssninja.io/" target="_blank">
                            Docs
                        </a>
                        <!-- Navbar item -->
                        <a class="navbar-item is-slide" href="https://cssninja.io/" target="_blank">
                            Help
                        </a>
                    </div>

                    <!-- Navbar end -->
                    <div class="navbar-end">
                        <!-- Signup button -->
                        <div class="navbar-item">
                            <a id="#signup-btn" href="https://1.envato.market/c/1288816/275988/4415?subId1=button&subId2=banner&subId3=https%3A%2F%2Fthemeforest.net%2Fitem%2Fbulkit-agency-startup-and-saas-template%2F21278677&u=https%3A%2F%2Fthemeforest.net%2Fitem%2Fbulkit-agency-startup-and-saas-template%2F21278677" class="button button-cta btn-outlined is-bold btn-align light-btn rounded raised">
                                Buy now
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </nav>
        <!-- Hero image -->
        <div id="main-hero" class="hero-body">
            <div class="container has-text-centered">
                <div class="columns is-vcentered">
                    <div class="column is-4 is-offset-1 has-text-left">
                        <h1 class="title components-title is-1">
                            Cards
                        </h1>
                        <h2 class="subtitle is-4 components-subtitle">
                            Card styles and variations.
                        </h2>
                        <p class="components-cta">
                            <a href="_components-basicui-cards.php#flex-card" class="button button-cta btn-align light-btn btn-outlined is-bold rounded">
                                Get Started
                            </a>
                        </p>
                    </div>
                    <div class="column is-offset-1 has-text-centered is-hidden-mobile">
                        <div id="cards-ill" class="vivus-svg"></div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Scrollnav -->
    <div id="scrollnav" class="scroll-nav-wrapper">
        <div class="container">
            <div class="tabs scrollnav-tabs is-centered">
                <ul>
                    <li class="scrollnav-item is-active"><a href="_components-basicui-cards.php#flex-card">Flex card</a></li>
                    <li class="scrollnav-item"><a href="_components-basicui-cards.php#feature-card">Feature card</a></li>
                    <li class="scrollnav-item"><a href="_components-basicui-cards.php#bulma-card">Classic card</a></li>
                    <li class="scrollnav-item"><a href="_components-basicui-cards.php#media-card">Media card</a></li>
                    <li class="scrollnav-item"><a href="_components-basicui-cards.php#event-card">Event card</a></li>
                </ul>
            </div>
        </div>
    </div>


    <div class="section">
        <div class="container">
            <div id="flex-card" class="section-title-wrapper has-text-centered">
                <div class="bg-number">1</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Flex-card
                </h2>
            </div>

            <!-- Flex card basic -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Basic flex card</h4>
                        <p class="no-padding">Flex card is a very versatile card component. It automatically spans the width of its container. you can do many things with this type of card and display any type of content, wether on a website or in an app.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-4 is-offset-2">
                    <div class="flex-card simple-shadow">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="flex-card simple-shadow">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card simple-shadow&#x22;&#x3E;
            &#x3C;div class=&#x22;card-body&#x22;&#x3E;
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    &#x3C;h2 class=&#x22;no-margin&#x22;&#x3E;Card title&#x3C;/h2&#x3E;
                    &#x3C;p class=&#x22;pt-10&#x22;&#x3E;Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.&#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Flex card light-bordered -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Light bordered</h4>
                        <p class="no-padding">You can display a flex card with a light border by adding the <code>.light-bordered</code> class.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-4 is-offset-2">
                    <div class="flex-card light-bordered">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="flex-card light-bordered">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered mt-20">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card light-bordered&#x22;&#x3E;
            &#x3C;div class=&#x22;card-body&#x22;&#x3E;
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    &#x3C;h2 class=&#x22;no-margin title-bold&#x22;&#x3E;Card title&#x3C;/h2&#x3E;
                    &#x3C;p class=&#x22;pt-10&#x22;&#x3E;Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.&#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Raised cards -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-4 is-offset-2">
                    <!-- V1 section -->
                    <div class="info-block">
                        <h4>Light raised</h4>
                        <p>You can display a flex card with a light box shadow by adding the <code>.light-raised</code> class.</p>
                    </div>

                    <div class="flex-card light-bordered light-raised">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <!-- V1 section -->
                    <div class="info-block">
                        <h4>Raised</h4>
                        <p>You can display a more important box shadow by adding the <code>.raised</code> class.</p>
                    </div>

                    <div class="flex-card light-bordered raised">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <div class="navigation-tabs animated-tabs simple-tabs mb-40">
                                    <div class="tabs is-right">
                                        <ul>
                                            <li class="tab-link is-active" data-tab="light-raised"><a>light raised</a></li>
                                            <li class="tab-link" data-tab="raised"><a>raised</a></li>
                                        </ul>
                                    </div>
                                    <div id="light-raised" class="navtab-content is-active">
                                        <figure class="highlight highlight-block"><pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card light-bordered light-raised&#x22;&#x3E;
            &#x3C;div class=&#x22;card-body&#x22;&#x3E;
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    &#x3C;h2 class=&#x22;no-margin title-bold&#x22;&#x3E;Card title&#x3C;/h2&#x3E;
                    &#x3C;p class=&#x22;pt-10&#x22;&#x3E;Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.&#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                    </code></pre>
                                        </figure>
                                    </div>
                                    <div id="raised" class="navtab-content">
                                        <figure class="highlight highlight-block"><pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card light-bordered raised&#x22;&#x3E;
            &#x3C;div class=&#x22;card-body&#x22;&#x3E;
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    &#x3C;h2 class=&#x22;no-margin title-bold&#x22;&#x3E;Card title&#x3C;/h2&#x3E;
                    &#x3C;p class=&#x22;pt-10&#x22;&#x3E;Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.&#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                    </code></pre>
                                        </figure>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Flex card hover raised -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-4 is-offset-4">
                    <!-- V1 section -->
                    <div class="info-block">
                        <h4>Hover raised</h4>
                        <p>You can make a card raise on hover by adding the <code>.hover-raised</code> class.</p>
                    </div>

                    <div class="flex-card light-bordered hover-raised">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Hover me</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                    <!-- /V1 section -->

                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card light-bordered hover-raised&#x22;&#x3E;
            &#x3C;div class=&#x22;card-body&#x22;&#x3E;
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    &#x3C;h2 class=&#x22;no-margin title-bold&#x22;&#x3E;Card title&#x3C;/h2&#x3E;
                    &#x3C;p class=&#x22;pt-10&#x22;&#x3E;Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.&#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Flex card hover inset -->
            <div class="columns is-vcentered mt-80">
                <div class="column is-4 is-offset-4">
                    <!-- V1 section -->
                    <div class="info-block pb-20">
                        <h4>Hover inset</h4>
                        <p>You give some inset to a card on hover by adding the class <code>.hover-inset</code>.</p>
                    </div>

                    <div class="flex-card light-bordered hover-inset">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Hover me</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>

                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card light-bordered hover-inset&#x22;&#x3E;
            &#x3C;div class=&#x22;card-body&#x22;&#x3E;
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    &#x3C;h2 class=&#x22;no-margin title-bold&#x22;&#x3E;Card title&#x3C;/h2&#x3E;
                    &#x3C;p class=&#x22;pt-10&#x22;&#x3E;Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.&#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Flex card overflow -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <span class="tag is-secondary">Reworked 3.0.0</span>
                        <h4>Overflow card</h4>
                        <p class="no-padding">If you plan to design cards with elements getting out of the flow, just add the <code>.card-overflow</code> class to the flex card element.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <div class="column is-5 is-offset-1">
                    <!-- V1 section -->
                    <div class="demo">
                        <div class="testimonial-item">
                            <div class="flex-card card-overflow raised mb-40">
                                <div class="testimonial-avatar">
                                    <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/carolin.png">
                                </div>
                                <div class="testimonial-name">
                                    <h3>Marjory Cambell</h3>
                                    <span>CEO</span>
                                </div>
                                <div class="testimonial-content">
                                    <p>Lorem ipsum dolor sit amet, clita laoreet ne cum. His cu harum inermis iudicabit. Ex vidit fierent hendrerit eum, sed stet periculis ut. Vis in probo decore labitur. Unum simul an vis, tale patrioque eos ad, dicunt percipit ea nam.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-5">
                    <!-- V1 section -->
                    <div class="demo">
                        <div class="testimonial-item">
                            <div class="flex-card card-overflow raised mb-40">
                                <div class="testimonial-avatar">
                                    <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/helen.jpg">
                                </div>
                                <div class="testimonial-name">
                                    <h3>Helen Miller</h3>
                                    <span>Sales team</span>
                                </div>
                                <div class="testimonial-content">
                                    <p>Lorem ipsum dolor sit amet, clita laoreet ne cum. His cu harum inermis iudicabit. Ex vidit fierent hendrerit eum, sed stet periculis ut. Vis in probo decore labitur. Unum simul an vis, tale patrioque eos ad, dicunt percipit ea nam.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Card colors -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-4">
                    <!-- V1 section -->
                    <div class="info-block">
                        <h4>Primary card</h4>
                        <p>Give a card a primary background by adding the <code>.primary-card</code> class.</p>
                    </div>

                    <div class="flex-card light-bordered primary-card light-raised">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <!-- V1 section -->
                    <div class="info-block">
                        <h4>Secondary card</h4>
                        <p>Give a card a secondary background by adding the <code>.secondary-card</code> class.</p>
                    </div>

                    <div class="flex-card light-bordered secondary-card light-raised">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <!-- V1 section -->
                    <div class="info-block">
                        <h4>Accent card</h4>
                        <p>Give a card an accent background by adding the <code>.accent-card</code> class.</p>
                    </div>

                    <div class="flex-card light-bordered accent-card light-raised">
                        <div class="card-body">
                            <div class="content">
                                <h2 class="no-margin title-bold">Card title</h2>
                                <p class="pt-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Flex card header -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <!-- V1 section -->
                    <div class="info-block">
                        <h4>Flex card header</h4>
                        <p>You can add a header with content to any flex card.</p>
                    </div>

                    <div class="flex-card light-bordered hover-inset">
                        <div class="flex-card-header">
                            <div class="header-avatar">
                                <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/helen.jpg">
                            </div>
                            <div class="header-info">
                                <div class="header-title">Helen Miller</div>
                                <div class="header-subtitle">Sales team</div>
                            </div>
                            <div class="header-control ml-auto">
                                <i class="sl sl-icon-arrow-down"></i>
                            </div>
                        </div>
                        <div class="card-body is-small">
                            <div class="content">
                                <p class="pb-10">Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.</p>
                            </div>
                            <div class="has-text-right">
                                <a class="button btn-align btn-outlined accent-btn rounded raised">Follow</a>
                            </div>
                        </div>
                    </div>

                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card light-bordered hover-inset&#x22;&#x3E;
            &#x3C;div class=&#x22;flex-card-header&#x22;&#x3E;
                &#x3C;div class=&#x22;header-avatar&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/avatars/helen.jpg&#x22;&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;header-info&#x22;&#x3E;
                    &#x3C;div class=&#x22;header-title&#x22;&#x3E;Helen Miller&#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;header-subtitle&#x22;&#x3E;Sales team&#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;header-control ml-auto&#x22;&#x3E;
                    &#x3C;i class=&#x22;sl sl-icon-arrow-down&#x22;&#x3E;&#x3C;/i&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-body is-small&#x22;&#x3E;
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    &#x3C;p class=&#x22;pb-10&#x22;&#x3E;Lorem ipsum dolor sit amet, has vero libris melius ea, omnis velit comprehensam duo an, meis ornatus eam ad. Ius ut labores scaevola.&#x3C;/p&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;has-text-right&#x22;&#x3E;
                    &#x3C;a class=&#x22;button btn-align btn-outlined accent-btn rounded raised&#x22;&#x3E;Follow&#x3C;/a&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="section section-feature-grey">
        <div class="container">
            <div id="feature-card" class="section-title-wrapper has-text-centered">
                <div class="bg-number">2</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Feature card
                </h2>
            </div>

            <!-- Basic feature card -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Basic feature card</h4>
                        <p class="no-padding">Unlike flex card, feature card's dimensions are static. By default it has <code>300px320px </code> dimensions. Height can be extended by adding the modifiers <code>.card-md</code> <code>.card-lg</code> to the parent element. Feature cards display perfectly also on mobile.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-4">
                    <div class="feature-card light-bordered has-text-centered"></div>
                </div>
                <div class="column is-4">
                    <div class="feature-card light-bordered has-text-centered"></div>
                </div>
                <div class="column is-4">
                    <div class="feature-card light-bordered has-text-centered"></div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- Code section -->
                <div class="column is-6 is-offset-3">
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;feature-card light-bordered has-text-centered&#x22;&#x3E;&#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Icon feature card -->
            <div class="columns is-vcentered  pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Icon feature card</h4>
                        <p class="no-padding">Unlike flex card, feature card's dimensions are static. By default it has <code>300px x 320px </code> dimensions. Height can be extended by adding the modifier <code>.card-md</code> to the parent element. Feature cards display perfectly also on mobile.</p>
                    </div>
                </div>
            </div>

            <!-- Feature card -->
            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-one-third">
                    <div class="feature-card card-md light-bordered hover-inset has-text-centered mb-20">
                        <div class="card-icon">
                            <img src="assets/img/graphics/icons/demo/upload.svg" alt="">
                        </div>
                        <div class="card-title">
                            <h4>Some title</h4>
                        </div>
                        <div class="card-feature-description">
                            <span class="">Lorem ipsum dolor sit amet, clita laoreet ne cum. His caelus elet cu harum inermis iudicabit.</span>
                        </div>
                        <a href="_components-basicui-cards.php#" class="button btn-align btn-more is-link color-primary">
                            Learn more <i class="sl sl-icon-arrow-right"></i>
                        </a>
                    </div>
                </div>
                <div class="column">
                    <div class="feature-card card-md light-bordered hover-inset has-text-centered mb-20">
                        <div class="card-icon">
                            <img src="assets/img/graphics/icons/demo/wallet.svg" alt="">
                        </div>
                        <div class="card-title">
                            <h4>Some title</h4>
                        </div>
                        <div class="card-feature-description">
                            <span class="">Lorem ipsum dolor sit amet, clita laoreet ne cum. His caelus elet cu harum inermis iudicabit.</span>
                        </div>
                        <a href="_components-basicui-cards.php#" class="button btn-align btn-more is-link color-primary">
                            Learn more <i class="sl sl-icon-arrow-right"></i>
                        </a>
                    </div>
                </div>
                <div class="column">
                    <div class="feature-card card-md light-bordered hover-inset has-text-centered mb-20">
                        <div class="card-icon">
                            <img src="assets/img/graphics/icons/demo/building.svg" alt="">
                        </div>
                        <div class="card-title">
                            <h4>Some title</h4>
                        </div>
                        <div class="card-feature-description">
                            <span class="">Lorem ipsum dolor sit amet, clita laoreet ne cum. His caelus elet cu harum inermis iudicabit.</span>
                        </div>
                        <a href="_components-basicui-cards.php#" class="button btn-align btn-more is-link color-primary">
                            Learn more <i class="sl sl-icon-arrow-right"></i>
                        </a>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- Code section -->
                <div class="column is-6 is-offset-3">
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;feature-card card-md light-bordered hover-inset has-text-centered&#x22;&#x3E;
            &#x3C;div class=&#x22;card-icon&#x22;&#x3E;
                &#x3C;img src=&#x22;assets/img/graphics/icons/wallet-core.svg&#x22; alt=&#x22;&#x22;&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-title&#x22;&#x3E;
                &#x3C;h4&#x3E;Some title&#x3C;/h4&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-feature-description&#x22;&#x3E;
                &#x3C;span class=&#x22;&#x22;&#x3E;Lorem ipsum dolor sit amet, clita laoreet ne cum. His caelus elet cu harum inermis iudicabit.&#x3C;/span&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;a href=&#x22;#&#x22; class=&#x22;button btn-align btn-more is-link color-primary&#x22;&#x3E;
                Learn more &#x3C;i class=&#x22;sl sl-icon-arrow-right&#x22;&#x3E;&#x3C;/i&#x3E;
            &#x3C;/a&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Image feature card -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Image feature card</h4>
                        <p class="no-padding">Unlike flex card, feature card's dimensions are static. By default it has <code>300px x 320px </code> dimensions. Height can be extended by adding the modifier <code>.card-md</code> to the parent element. Feature cards display perfectly also on mobile.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-4">
                    <div class="feature-card card-lg light-bordered hover-inset mb-20">
                        <div class="card-img">
                            <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/cactus.png">
                        </div>
                        <div class="card-title pb-10 pt-10">
                            <h4>Some title</h4>
                        </div>
                        <div class="card-img-description has-text-left">
                            <span class="">Lorem ipsum dolor sit amet, clita laoreet ne cum. His caelus elet cu harum Cu soleat euismod mei, illum soleat ei vis.</span>
                        </div>
                        <div class="pt-10 has-text-left">
                            <a href="_components-basicui-cards.php#" class="button btn-align secondary-btn raised">Learn more</a>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="feature-card card-lg light-bordered hover-inset mb-20">
                        <div class="card-img">
                            <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/moon.png">
                        </div>
                        <div class="card-title pb-10 pt-10">
                            <h4>Some title</h4>
                        </div>
                        <div class="card-img-description has-text-left">
                            <span class="">Lorem ipsum dolor sit amet, clita laoreet ne cum. His caelus elet cu harum Cu soleat euismod mei, illum soleat ei vis.</span>
                        </div>
                        <div class="pt-10 has-text-left">
                            <a href="_components-basicui-cards.php#" class="button btn-align secondary-btn raised">Learn more</a>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="feature-card card-lg light-bordered hover-inset mb-20">
                        <div class="card-img">
                            <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/working.png">
                        </div>
                        <div class="card-title pb-10 pt-10">
                            <h4>Some title</h4>
                        </div>
                        <div class="card-img-description has-text-left">
                            <span class="">Lorem ipsum dolor sit amet, clita laoreet ne cum. His caelus elet cu harum Cu soleat euismod mei, illum soleat ei vis.</span>
                        </div>
                        <div class="pt-10 has-text-left">
                            <a href="_components-basicui-cards.php#" class="button btn-align secondary-btn raised">Learn more</a>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered pb-60">
                <!-- Code section -->
                <div class="column is-6 is-offset-3">
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;feature-card card-lg light-bordered hover-inset&#x22;&#x3E;
            &#x3C;div class=&#x22;card-img&#x22;&#x3E;
                &#x3C;img src=&#x22;assets/img/photos/moon.png&#x22; alt=&#x22;&#x22;&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-title  pb-10 pt-10&#x22;&#x3E;
                &#x3C;h4&#x3E;Some title&#x3C;/h4&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-img-description has-text-left&#x22;&#x3E;
                &#x3C;span class=&#x22;&#x22;&#x3E;Lorem ipsum dolor sit amet, clita laoreet ne cum. His caelus elet cu harum Cu soleat euismod mei, illum soleat ei vis, offendit partiendo intellegebat id vix. ...&#x3C;/span&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="section">
        <div class="container">
            <div id="bulma-card" class="section-title-wrapper has-text-centered">
                <div class="bg-number">3</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Classic card
                </h2>
            </div>

            <!-- Image card -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Bulma cards</h4>
                        <p class="no-padding">Bulma natively offers wonderful image cards, wich are customizable. If you want to find more about Bulma cards, head to the official <a href="http://bulma.io/documentation/components/card/" target="_blank">cards documentation</a>.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-4">
                    <div class="card">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/cactus.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-left">
                                    <figure class="image is-48x48">
                                        <img src="https://via.placeholder.com/250x250" class="img-circle" alt="" data-demo-src="assets/img/avatars/helen.jpg">
                                    </figure>
                                </div>
                                <div class="media-content">
                                    <p class="is-size-4 color-accent">Helen Miller</p>
                                    <p class="is-size-6 muted-text">23 mar 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-right">
                                <a class="button btn-align accent-btn btn-outlined rounded">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="card">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/moon.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-left">
                                    <figure class="image is-48x48">
                                        <img src="https://via.placeholder.com/250x250" class="img-circle" alt="" data-demo-src="assets/img/avatars/ben.jpg">
                                    </figure>
                                </div>
                                <div class="media-content">
                                    <p class="is-size-4 color-accent">Kevin Smith</p>
                                    <p class="is-size-6 muted-text">12 apr 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-right">
                                <a class="button btn-align accent-btn btn-outlined rounded">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="card">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/working.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-left">
                                    <figure class="image is-48x48">
                                        <img src="https://via.placeholder.com/250x250" class="img-circle" alt="" data-demo-src="assets/img/avatars/nina.jpg">
                                    </figure>
                                </div>
                                <div class="media-content">
                                    <p class="is-size-4 color-accent">Sandra Mayers</p>
                                    <p class="is-size-6 muted-text">29 apr 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-right">
                                <a class="button btn-align accent-btn btn-outlined rounded">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Ressource card -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Ressource cards</h4>
                        <p class="no-padding">Ressource cards are useful when displaying ressources, articles or blog post. They feature a image zoom out and zoom in effect when hovered. To activate these effects, add <code>.zoomIn</code> or <code>.zoomOut</code> to the <code>figure</code> container</p>
                    </div>
                </div>
            </div>

            <!-- V1 section -->
            <div class="columns is-vcentered">
                <!-- Card -->
                <div class="column is-4">
                    <div class="card ressource-card">
                        <div class="card-image">
                            <figure class="image is-4by3 zoomIn">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/big-moon.jpg">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <a href="_components-basicui-cards.php#" class="color-primary">Work with less inbound emails</a>
                                    <p class="subtitle is-6">Learning Center</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Card -->
                <div class="column is-4">
                    <div class="card ressource-card">
                        <div class="card-image">
                            <figure class="image is-4by3 zoomIn">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/mountain-sunset.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <a href="_components-basicui-cards.php#" class="color-primary">Tight collaboration equals success</a>
                                    <p class="subtitle is-6">Learning Center</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Card -->
                <div class="column is-4">
                    <div class="card ressource-card">
                        <div class="card-image">

                            <figure class="image is-4by3 zoomIn">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/moon-mountains.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <a href="_components-basicui-cards.php#" class="color-primary">How data insight helps business</a>
                                    <p class="subtitle is-6">Blog</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- Code section -->
                <div class="column is-6 is-offset-3">
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;card ressource-card&#x22;&#x3E;
            &#x3C;div class=&#x22;card-image&#x22;&#x3E;
                &#x3C;figure class=&#x22;image is-4by3 zoomOut&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/photos/city.png&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/figure&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-content&#x22;&#x3E;
                &#x3C;div class=&#x22;media&#x22;&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;a href=&#x22;#&#x22; class=&#x22;color-primary&#x22;&#x3E;How data insight helps business&#x3C;/a&#x3E;
                        &#x3C;p class=&#x22;subtitle is-6&#x22;&#x3E;Blog&#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Avatar ressource card -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Avatar Ressource cards</h4>
                        <p class="no-padding">Avatars can be added to ressource cards. It is a useful feature when showing articles with multiple authors.</p>
                    </div>
                </div>
            </div>

            <!-- V1 section -->
            <div class="columns is-vcentered">
                <!-- Card -->
                <div class="column is-4">
                    <div class="card ressource-card">
                        <div class="card-image">
                            <figure class="image is-4by3 zoomIn">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/big-moon.jpg">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <a href="_components-basicui-cards.php#" class="color-primary">Work with less inbound emails</a>
                                    <p class="subtitle is-6">Learning Center</p>
                                </div>
                            </div>
                        </div>
                        <div class="ressource-avatar">
                            <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/funny1.svg">
                        </div>
                    </div>
                </div>

                <!-- Card -->
                <div class="column is-4">
                    <div class="card ressource-card">
                        <div class="card-image">
                            <figure class="image is-4by3 zoomIn">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/mountain-sunset.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <a href="_components-basicui-cards.php#" class="color-primary">Tight collaboration equals success</a>
                                    <p class="subtitle is-6">Learning Center</p>
                                </div>
                            </div>
                        </div>
                        <div class="ressource-avatar">
                            <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/funny2.svg">
                        </div>
                    </div>
                </div>

                <!-- Card -->
                <div class="column is-4">
                    <div class="card ressource-card">
                        <div class="card-image">

                            <figure class="image is-4by3 zoomIn">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/moon-mountains.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <a href="_components-basicui-cards.php#" class="color-primary">How data insight helps business</a>
                                    <p class="subtitle is-6">Blog</p>
                                </div>
                            </div>
                        </div>
                        <div class="ressource-avatar">
                            <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/funny3.svg">
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- Code section -->
                <div class="column is-6 is-offset-3">
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;card ressource-card&#x22;&#x3E;
            &#x3C;div class=&#x22;card-image&#x22;&#x3E;
                &#x3C;figure class=&#x22;image is-4by3 zoomIn&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/photos/moon-mountains.png&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/figure&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-content&#x22;&#x3E;
                &#x3C;div class=&#x22;media&#x22;&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;a href=&#x22;#&#x22; class=&#x22;color-primary&#x22;&#x3E;Tight collaboration equals success&#x3C;/a&#x3E;
                        &#x3C;p class=&#x22;subtitle is-6&#x22;&#x3E;Learning Center&#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;ressource-avatar&#x22;&#x3E;
                &#x3C;img src=&#x22;assets/img/avatars/funny2.svg&#x22;&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                    </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Floating Image card wide -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Floating wide image</h4>
                        <p class="no-padding">Floating image cards extend Bulma cards. They feature a floating wide image or a boxed one. Check the example markup for more details.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-4">
                    <div class="card card-floating-wide card-shadow">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/cactus.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <p class="title is-4 color-accent">New perspectives</p>
                                    <p class="subtitle is-6">23 mar 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-left">
                                <a class="button btn-align accent-btn raised">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="card card-floating-wide card-shadow">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/moon.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <p class="title is-4 color-accent">New perspectives</p>
                                    <p class="subtitle is-6">23 mar 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-left">
                                <a class="button btn-align accent-btn raised">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="card card-floating-wide card-shadow">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/working.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <p class="title is-4 color-accent">New perspectives</p>
                                    <p class="subtitle is-6">23 mar 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-left">
                                <a class="button btn-align accent-btn raised">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="columns is-vcentered">
                <!-- Code section -->
                <div class="column is-6 is-offset-3">
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;card card-floating-wide card-shadow&#x22;&#x3E;
            &#x3C;div class=&#x22;card-image&#x22;&#x3E;
                &#x3C;figure class=&#x22;image is-4by3&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/photos/cactus.png&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/figure&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-content&#x22;&#x3E;
                &#x3C;div class=&#x22;media&#x22;&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;p class=&#x22;title is-4 color-accent&#x22;&#x3E;New perspectives&#x3C;/p&#x3E;
                        &#x3C;p class=&#x22;subtitle is-6&#x22;&#x3E;23 mar 2017&#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
        
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                    Phasellus nec iaculis mauris.
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;has-text-left&#x22;&#x3E;
                    &#x3C;a class=&#x22;button btn-align accent-btn raised&#x22;&#x3E;Read more&#x3C;/a&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                    </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Floating Image card boxed -->
            <div class="columns is-vcentered pt-60 pb-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Floating boxed image</h4>
                        <p class="no-padding">Floating image cards extend Bulma cards. They feature a floating wide image or a boxed one. Check the example markup for more details.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-4">
                    <div class="card card-floating-boxed card-shadow">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/cactus.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <p class="title is-4 color-accent">New perspectives</p>
                                    <p class="subtitle is-6">23 mar 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-left">
                                <a class="button btn-align accent-btn raised">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="card card-floating-boxed card-shadow">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/moon.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <p class="title is-4 color-accent">New perspectives</p>
                                    <p class="subtitle is-6">23 mar 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-left">
                                <a class="button btn-align accent-btn raised">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <div class="card card-floating-boxed card-shadow">
                        <div class="card-image">
                            <figure class="image is-4by3">
                                <img src="https://via.placeholder.com/400x300" alt="" data-demo-src="assets/img/demo/flat/working.png">
                            </figure>
                        </div>
                        <div class="card-content">
                            <div class="media">
                                <div class="media-content">
                                    <p class="title is-4 color-accent">New perspectives</p>
                                    <p class="subtitle is-6">23 mar 2017</p>
                                </div>
                            </div>

                            <div class="content">
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                                Phasellus nec iaculis mauris.
                            </div>
                            <div class="has-text-left">
                                <a class="button btn-align accent-btn raised">Read more</a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="columns is-vcentered pb-60">
                <!-- Code section -->
                <div class="column is-6 is-offset-3">
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;card card-floating-boxed card-shadow&#x22;&#x3E;
            &#x3C;div class=&#x22;card-image&#x22;&#x3E;
                &#x3C;figure class=&#x22;image is-4by3&#x22;&#x3E;
                    &#x3C;img src=&#x22;assets/img/photos/cactus.png&#x22; alt=&#x22;&#x22;&#x3E;
                &#x3C;/figure&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-content&#x22;&#x3E;
                &#x3C;div class=&#x22;media&#x22;&#x3E;
                    &#x3C;div class=&#x22;media-content&#x22;&#x3E;
                        &#x3C;p class=&#x22;title is-4 color-accent&#x22;&#x3E;New perspectives&#x3C;/p&#x3E;
                        &#x3C;p class=&#x22;subtitle is-6&#x22;&#x3E;23 mar 2017&#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
        
                &#x3C;div class=&#x22;content&#x22;&#x3E;
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                    Phasellus nec iaculis mauris.
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;has-text-left&#x22;&#x3E;
                    &#x3C;a class=&#x22;button btn-align accent-btn raised&#x22;&#x3E;Read more&#x3C;/a&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                    </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="section section-feature-grey">
        <div class="container">
            <div id="media-card" class="section-title-wrapper has-text-centered">
                <div class="bg-number">4</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Media card
                </h2>
            </div>
            <!-- Left Media card -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Left media card</h4>
                        <p class="no-padding">Inspired by <a href="_components-basicui-cards.php#">Medium</a> and built on top of flex cards, media cards are a great alternative way to display your content. The side layout automatically stacks on touch devices.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-5 is-offset-1">
                    <div class="flex-card media-card media-card-left light-bordered light-raised">
                        <div class="columns is-gapless is-desktop">
                            <div class="column is-4 media-stretch">
                                <div class="media-card-image" data-background="https://via.placeholder.com/1280x960" data-demo-background="assets/img/demo/woman-glasses.jpeg"></div>
                            </div>
                            <div class="column content-column">
                                <div class="mcard-content">
                                    <h3 class="mcard-title"><a href="_components-basicui-cards.php#">How SaaS can help small businesses becoming big players?</a></h3>
                                    <p class="mcard-description is-hidden-touch">When it comes to structuring a React app, the ideal structure is the one that allows you to move around your code... </p>
                                </div>
                                <div class="mcard-controls">
                                    <div class="mcard-avatar">
                                        <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/henry.jpg">
                                    </div>
                                    <div class="mcard-info">
                                        <span><a href="_components-basicui-cards.php#">Henry Rogers</a></span>
                                        <div><span>13 Oct</span> <i class="fa fa-circle"></i> <span>8 min read</span></div>
                                    </div>
                                    <div class="mcard-actions ml-auto">
                                        <span><i class="material-icons">bookmark_border</i> </span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-5">
                    <div class="flex-card media-card media-card-left light-bordered light-raised">
                        <div class="columns is-gapless is-desktop">
                            <div class="column is-4 media-stretch">
                                <div class="media-card-image" data-background="https://via.placeholder.com/1280x960" data-demo-background="assets/img/demo/woman-glasses-back.jpeg"></div>
                            </div>
                            <div class="column content-column">
                                <div class="mcard-content">
                                    <h3 class="mcard-title"><a href="_components-basicui-cards.php#">Integrate our cities with technology, a major concern.</a></h3>
                                    <p class="mcard-description is-hidden-touch">When it comes to structuring a React app, the ideal structure is the one that allows you to move around your code... </p>
                                </div>
                                <div class="mcard-controls">
                                    <div class="mcard-avatar">
                                        <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/janet.jpg">
                                    </div>
                                    <div class="mcard-info">
                                        <span><a href="_components-basicui-cards.php#">Janet Biggs</a></span>
                                        <div><span>06 Oct</span> <i class="fa fa-circle"></i> <span>4 min read</span></div>
                                    </div>
                                    <div class="mcard-actions ml-auto">
                                        <span><i class="material-icons">bookmark_border</i> </span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Right Media card -->
            <div class="columns is-vcentered pt-60">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Right media card</h4>
                        <p class="no-padding">Inspired by <a href="_components-basicui-cards.php#">Medium</a> and built on top of flex cards, media cards are a great alternative way to display your content. The side layout automatically stacks on touch devices.</p>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered">
                <!-- V1 section -->
                <div class="column is-5 is-offset-1">
                    <div class="flex-card media-card media-card-right light-bordered light-raised">
                        <div class="columns is-gapless is-desktop">
                            <div class="column content-column">
                                <div class="mcard-content">
                                    <h3 class="mcard-title"><a href="_components-basicui-cards.php#">How to start a successfull React project?</a></h3>
                                    <p class="mcard-description is-hidden-touch">When it comes to structuring a React app, the ideal structure is the one that allows you to move around your code... </p>
                                </div>
                                <div class="mcard-controls">
                                    <div class="mcard-avatar">
                                        <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/melany.jpg">
                                    </div>
                                    <div class="mcard-info">
                                        <span><a href="_components-basicui-cards.php#">Melany Lane</a></span>
                                        <div><span>29 Aug</span> <i class="fa fa-circle"></i> <span>15 min read</span></div>
                                    </div>
                                    <div class="mcard-actions ml-auto">
                                        <span><i class="material-icons">bookmark_border</i> </span>
                                    </div>
                                </div>
                            </div>
                            <div class="column is-4 media-stretch">
                                <div class="media-card-image" data-background="https://via.placeholder.com/1280x960" data-demo-background="assets/img/demo/laptop-living.jpeg"></div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-5">
                    <div class="flex-card media-card media-card-right light-bordered light-raised">
                        <div class="columns is-gapless is-desktop">
                            <div class="column content-column">
                                <div class="mcard-content">
                                    <h3 class="mcard-title"><a href="_components-basicui-cards.php#">Tomorrow, smart cities will rule the earth.</a></h3>
                                    <p class="mcard-description is-hidden-touch">When it comes to structuring a React app, the ideal structure is the one that allows you to move around your code... </p>
                                </div>
                                <div class="mcard-controls">
                                    <div class="mcard-avatar">
                                        <img src="https://via.placeholder.com/250x250" alt="" data-demo-src="assets/img/avatars/janet.jpg">
                                    </div>
                                    <div class="mcard-info">
                                        <span><a href="_components-basicui-cards.php#">Janet Biggs</a></span>
                                        <div><span>06 Oct</span> <i class="fa fa-circle"></i> <span>4 min read</span></div>
                                    </div>
                                    <div class="mcard-actions ml-auto">
                                        <span><i class="material-icons">bookmark_border</i> </span>
                                    </div>
                                </div>
                            </div>
                            <div class="column is-4 media-stretch">
                                <div class="media-card-image" data-background="https://via.placeholder.com/1280x960" data-demo-background="assets/img/demo/woman-ipad.jpg"></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered pb-60">
                <div class="column is-6 is-offset-3">
                    <!-- Code section -->
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <div class="navigation-tabs animated-tabs simple-tabs mb-40">
                                    <div class="tabs is-right">
                                        <ul>
                                            <li class="tab-link is-active" data-tab="left-media"><a>left media</a></li>
                                            <li class="tab-link" data-tab="right-media"><a>right media</a></li>
                                        </ul>
                                    </div>
                                    <div id="left-media" class="navtab-content is-active">
                                        <figure class="highlight highlight-block"><pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card media-card media-card-left light-bordered light-raised&#x22;&#x3E;
            &#x3C;div class=&#x22;columns is-gapless is-desktop&#x22;&#x3E;
                &#x3C;div class=&#x22;column is-4 media-stretch&#x22;&#x3E;
                    &#x3C;div class=&#x22;media-card-image&#x22; data-background=&#x22;assets/img/photos/woman-glasses-back.jpeg&#x22;&#x3E;&#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;column content-column&#x22;&#x3E;
                    &#x3C;div class=&#x22;mcard-content&#x22;&#x3E;
                        &#x3C;h3 class=&#x22;mcard-title&#x22;&#x3E;&#x3C;a href=&#x22;#&#x22;&#x3E;Integrate our cities with technology, a major concern.&#x3C;/a&#x3E;&#x3C;/h3&#x3E;
                        &#x3C;p class=&#x22;mcard-description is-hidden-touch&#x22;&#x3E;When it comes to structuring a React app, the ideal structure is the one that allows you to move around your code... &#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;mcard-controls&#x22;&#x3E;
                        &#x3C;div class=&#x22;mcard-avatar&#x22;&#x3E;
                            &#x3C;img src=&#x22;assets/img/avatars/janet.jpg&#x22;&#x3E;
                        &#x3C;/div&#x3E;
                        &#x3C;div class=&#x22;mcard-info&#x22;&#x3E;
                            &#x3C;span&#x3E;&#x3C;a href=&#x22;#&#x22;&#x3E;Janet Biggs&#x3C;/a&#x3E;&#x3C;/span&#x3E;
                            &#x3C;div&#x3E;&#x3C;span&#x3E;06 Oct&#x3C;/span&#x3E; &#x3C;i class=&#x22;fa fa-circle&#x22;&#x3E;&#x3C;/i&#x3E;  &#x3C;span&#x3E;4 min read&#x3C;/span&#x3E;&#x3C;/div&#x3E;
                        &#x3C;/div&#x3E;
                        &#x3C;div class=&#x22;mcard-actions ml-auto&#x22;&#x3E;
                            &#x3C;span&#x3E;&#x3C;i class=&#x22;material-icons&#x22;&#x3E;bookmark_border&#x3C;/i&#x3E; &#x3C;/span&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                    </code></pre>
                                        </figure>
                                    </div>
                                    <div id="right-media" class="navtab-content">
                                        <figure class="highlight highlight-block"><pre><code class="html code-highlight">
        &#x3C;div class=&#x22;flex-card media-card media-card-right light-bordered light-raised&#x22;&#x3E;
            &#x3C;div class=&#x22;columns is-gapless is-desktop&#x22;&#x3E;
                &#x3C;div class=&#x22;column content-column&#x22;&#x3E;
                    &#x3C;div class=&#x22;mcard-content&#x22;&#x3E;
                        &#x3C;h3 class=&#x22;mcard-title&#x22;&#x3E;&#x3C;a href=&#x22;#&#x22;&#x3E;How to start a successfull React project?&#x3C;/a&#x3E;&#x3C;/h3&#x3E;
                        &#x3C;p class=&#x22;mcard-description is-hidden-touch&#x22;&#x3E;When it comes to structuring a React app, the ideal structure is the one that allows you to move around your code... &#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;mcard-controls&#x22;&#x3E;
                        &#x3C;div class=&#x22;mcard-avatar&#x22;&#x3E;
                            &#x3C;img src=&#x22;assets/img/avatars/melany.jpg&#x22;&#x3E;
                        &#x3C;/div&#x3E;
                        &#x3C;div class=&#x22;mcard-info&#x22;&#x3E;
                            &#x3C;span&#x3E;&#x3C;a href=&#x22;#&#x22;&#x3E;Melany Lane&#x3C;/a&#x3E;&#x3C;/span&#x3E;
                            &#x3C;div&#x3E;&#x3C;span&#x3E;29 Aug&#x3C;/span&#x3E; &#x3C;i class=&#x22;fa fa-circle&#x22;&#x3E;&#x3C;/i&#x3E;  &#x3C;span&#x3E;15 min read&#x3C;/span&#x3E;&#x3C;/div&#x3E;
                        &#x3C;/div&#x3E;
                        &#x3C;div class=&#x22;mcard-actions ml-auto&#x22;&#x3E;
                            &#x3C;span&#x3E;&#x3C;i class=&#x22;material-icons&#x22;&#x3E;bookmark_border&#x3C;/i&#x3E; &#x3C;/span&#x3E;
                        &#x3C;/div&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
                &#x3C;div class=&#x22;column is-4 media-stretch&#x22;&#x3E;
                    &#x3C;div class=&#x22;media-card-image&#x22; data-background=&#x22;assets/img/photos/laptop-phone.jpeg&#x22;&#x3E;&#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                    </code></pre>
                                        </figure>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>

    <div class="section">
        <div class="container">
            <div id="event-card" class="section-title-wrapper has-text-centered">
                <div class="bg-number">5</div>
                <h2 class="title section-title has-text-centered dark-text text-bold">
                    Event card
                </h2>
            </div>
            <!-- Event cards -->
            <div class="columns is-vcentered">
                <div class="column is-6 is-offset-3">
                    <div class="info-block">
                        <h4>Hoverable event cards</h4>
                        <p class="no-padding">A nice to have if you'd like to show some events or any content that should be revealed on hover.</p>
                    </div>
                </div>
            </div>

            <div class="columns">
                <div class="column is-4">
                    <!-- Event Card -->
                    <div class="event-card">
                        <div class="card-date">
                            <div class="date">
                                <span class="day">24</span>
                                <span class="month">Aug</span>
                            </div>
                        </div>
                        <div class="img-container">
                            <img src="https://via.placeholder.com/1080x810" alt="" data-demo-src="assets/img/demo/events/event1.jpg">
                        </div>
                        <div class="card-text">
                            <div class="text text-container">
                                <div class="text text-header">
                                    <h2 class="text text-title">Marbella</h2>
                                    <p class="text text-subtitle">Partners training session</p>
                                </div>
                                <div class="text text-details">
                                    <p class="text text-description">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ea nulla cum impedit et nemo molestiae, suscipit animi officiis soluta natus, provident ab laudantium, amet dolorum.
                                    </p>
                                    <a href="_components-basicui-cards.php#" class="button btn-align btn-more is-link color-accent mt-10">
                                        Event details <i class="sl sl-icon-arrow-right"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <!-- Event Card -->
                    <div class="event-card">
                        <div class="card-date">
                            <div class="date">
                                <span class="day">19</span>
                                <span class="month">Sep</span>
                            </div>
                        </div>
                        <div class="img-container">
                            <img src="https://via.placeholder.com/1080x810" alt="" data-demo-src="assets/img/demo/events/event2.jpg">
                        </div>
                        <div class="card-text">
                            <div class="text text-container">
                                <div class="text text-header">
                                    <h2 class="text text-title">Prague</h2>
                                    <p class="text text-subtitle">Release momentum</p>
                                </div>
                                <div class="text text-details">
                                    <p class="text text-description">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ea nulla cum impedit et nemo molestiae, suscipit animi officiis soluta natus, provident ab laudantium, amet dolorum.
                                    </p>
                                    <a href="_components-basicui-cards.php#" class="button btn-align btn-more is-link color-accent mt-10">
                                        Event details <i class="sl sl-icon-arrow-right"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="column is-4">
                    <!-- Event Card -->
                    <div class="event-card">
                        <div class="card-date">
                            <div class="date">
                                <span class="day">05</span>
                                <span class="month">Dec</span>
                            </div>
                        </div>
                        <div class="img-container">
                            <img src="https://via.placeholder.com/1080x810" alt="" data-demo-src="assets/img/demo/events/event3.svg">
                        </div>
                        <div class="card-text">
                            <div class="text text-container">
                                <div class="text text-header">
                                    <h2 class="text text-title">Chicago</h2>
                                    <p class="text text-subtitle">Node JS Conf</p>
                                </div>
                                <div class="text text-details">
                                    <p class="text text-description">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ea nulla cum impedit et nemo molestiae, suscipit animi officiis soluta natus, provident ab laudantium, amet dolorum.
                                    </p>
                                    <a href="_components-basicui-cards.php#" class="button btn-align btn-more is-link color-accent mt-10">
                                        Event details <i class="sl sl-icon-arrow-right"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <div class="columns is-vcentered pb-60">
                <!-- Code section -->
                <div class="column is-6 is-offset-3">
                    <div class="accordion-wrapper">
                        <div class="toggle-wrap">
                            <span class="trigger"><a href="_components-basicui-cards.php#"><i class="im im-icon-Coding is-icon-2x"></i> SHOW CODE<i class="im im-icon-Add"></i></a></span>
                            <div class="toggle-container">
                                <figure class="highlight highlight-block">
                                    <pre><code class="html code-highlight">
        &#x3C;div class=&#x22;event-card&#x22;&#x3E;
            &#x3C;div class=&#x22;card-date&#x22;&#x3E;
                &#x3C;div class=&#x22;date&#x22;&#x3E;
                    &#x3C;span class=&#x22;day&#x22;&#x3E;05&#x3C;/span&#x3E;
                    &#x3C;span class=&#x22;month&#x22;&#x3E;Dec&#x3C;/span&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;img-container&#x22;&#x3E;
                &#x3C;img src=&#x22;https://source.unsplash.com/tnv84LOjes4/&#x22; alt=&#x22;&#x22; /&#x3E;
            &#x3C;/div&#x3E;
            &#x3C;div class=&#x22;card-text&#x22;&#x3E;
                &#x3C;div class=&#x22;text text-container&#x22;&#x3E;
                    &#x3C;div class=&#x22;text text-header&#x22;&#x3E;
                        &#x3C;h2 class=&#x22;text text-title&#x22;&#x3E;Chicago&#x3C;/h2&#x3E;
                        &#x3C;p class=&#x22;text text-subtitle&#x22;&#x3E;Node JS Conf&#x3C;/p&#x3E;
                    &#x3C;/div&#x3E;
                    &#x3C;div class=&#x22;text text-details&#x22;&#x3E;
                        &#x3C;p class=&#x22;text text-description&#x22;&#x3E;Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ea nulla cum impedit et nemo molestiae, suscipit animi officiis soluta natus, provident ab laudantium, amet dolorum.
                        &#x3C;/p&#x3E;
                        &#x3C;a href=&#x22;#&#x22; class=&#x22;button btn-align btn-more is-link color-accent mt-10&#x22;&#x3E;
                            Event details &#x3C;i class=&#x22;sl sl-icon-arrow-right&#x22;&#x3E;&#x3C;/i&#x3E;
                        &#x3C;/a&#x3E;
                    &#x3C;/div&#x3E;
                &#x3C;/div&#x3E;
            &#x3C;/div&#x3E;
        &#x3C;/div&#x3E;
                                                    </code></pre>
                                </figure>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Dark footer -->
    <footer id="dark-footer" class="footer footer-dark">
        <div class="container">
            <div class="columns">
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Product</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-cards.php#">Discover features</a></li>
                            <li><a href="_components-basicui-cards.php#">CMS integration</a></li>
                            <li><a href="_components-basicui-cards.php#">Customers</a></li>
                            <li><a href="_components-basicui-cards.php#">Weekly sessions</a></li>
                            <li><a href="_components-basicui-cards.php#">Free trials and demo</a></li>
                            <li><a href="_components-basicui-cards.php#">What's next ?</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Company</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-cards.php#">About us</a></li>
                            <li><a href="_components-basicui-cards.php#">About security</a></li>
                            <li><a href="_components-basicui-cards.php#">User guide</a></li>
                            <li><a href="_components-basicui-cards.php#">Terms of service</a></li>
                            <li><a href="_components-basicui-cards.php#">Terms of website use</a></li>
                            <li><a href="_components-basicui-cards.php#">Privacy policy</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-header">
                            <h3>Learning</h3>
                        </div>
                        <ul class="link-list">
                            <li><a href="_components-basicui-cards.php#">Resources</a></li>
                            <li><a href="_components-basicui-cards.php#">Blog</a></li>
                            <li><a href="_components-basicui-cards.php#">FAQ</a></li>
                            <li><a href="_components-basicui-cards.php#">API documentation</a></li>
                            <li><a href="_components-basicui-cards.php#">Admin guide</a></li>
                        </ul>
                    </div>
                </div>
                <div class="column">
                    <div class="footer-column">
                        <div class="footer-logo">
                            <img class="switcher-logo-w" src="assets/img/logos/logo/bulkit-core-w.svg" alt="">
                        </div>
                        <div class="footer-header">
                            <nav class="level is-mobile">
                                <div class="level-left level-social">
                                    <a href="_components-basicui-cards.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-facebook"></i></span>
                                    </a>
                                    <a href="_components-basicui-cards.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-twitter"></i></span>
                                    </a>
                                    <a href="_components-basicui-cards.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-linkedin"></i></span>
                                    </a>
                                    <a href="_components-basicui-cards.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-dribbble"></i></span>
                                    </a>
                                    <a href="_components-basicui-cards.php#" class="level-item">
                                        <span class="icon"><i class="fa fa-github"></i></span>
                                    </a>
                                </div>
                            </nav>
                        </div>
                        <div class="copyright">
                            <span class="moto light-text">Designed and coded with <i class="fa fa-heart color-red"></i> by CSS Ninja.</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </footer>
    <!-- /Dark footer -->
    <!-- Side navigation -->
    <div class="side-navigation-menu">
        <!-- Categories menu -->
        <div class="category-menu-wrapper">
            <!-- Menu -->
            <ul class="categories">
                <li class="square-logo"><img src="assets/img/logos/square-white.svg" alt=""></li>
                <li class="category-link is-active" data-navigation-menu="demo-pages"><i class="sl sl-icon-layers"></i></li>
                <li class="category-link" data-navigation-menu="onepagers"><i class="sl sl-icon-docs"></i></li>
                <li class="category-link" data-navigation-menu="components"><i class="sl sl-icon-grid"></i></li>
            </ul>
            <!-- Menu -->

            <ul class="author">
                <li>
                    <!-- Theme author -->
                    <a href="https://cssninja.io" target="_blank">
                        <img class="main-menu-author" src="assets/img/logos/cssninja.svg" alt="">
                    </a>
                </li>
            </ul>
        </div>
        <!-- /Categories menu -->

        <!-- Navigation menu -->
        <div id="demo-pages" class="navigation-menu-wrapper animated preFadeInRight fadeInRight">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>V1 pages</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation Body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">weekend</span>Agency kit</a>
                    <ul>
                        <li><a class="is-submenu" href="agency-home.php">Homepage</a></li>
                        <li><a class="is-submenu" href="agency-about.php">About</a></li>
                        <li><a class="is-submenu" href="agency-portfolio.php">Portfolio</a></li>
                        <li><a class="is-submenu" href="agency-contact.php">Contact</a></li>
                        <li><a class="is-submenu" href="agency-blog.php">Blog</a></li>
                        <li><a class="is-submenu" href="agency-post-nosidebar.php">Post no sidebar</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">wb_incandescent</span>Startup kit</a>
                    <ul>
                        <li><a class="is-submenu" href="startup-landing.php">Homepage</a></li>
                        <li><a class="is-submenu" href="startup-about.php">About</a></li>
                        <li><a class="is-submenu" href="startup-product.php">Product</a></li>
                        <li><a class="is-submenu" href="startup-contact.php">Contact</a></li>
                        <li><a class="is-submenu" href="startup-login.php">Login</a></li>
                        <li><a class="is-submenu" href="startup-signup.php">Sign up</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">apps</span>Landing kit 1</a>
                    <ul>
                        <li><a class="is-submenu" href="kit1-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit1-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit1-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit1-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit1-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit1-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit1-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit1-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit1-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit1-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">timelapse</span>Landing kit 2</a>
                    <ul>
                        <li><a class="is-submenu" href="kit2-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit2-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit2-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit2-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit2-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit2-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit2-landing-7.php">Landing page v7</a></li>
                        <li><a class="is-submenu" href="kit2-landing-8.php">Landing page v8</a></li>
                        <li><a class="is-submenu" href="kit2-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit2-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit2-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit2-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">assistant</span>Landing kit 3</a>
                    <ul>
                        <li><a class="is-submenu" href="kit3-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit3-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit3-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit3-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit3-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit3-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">brightness_2</span>Landing kit 4</a>
                    <ul>
                        <li><a class="is-submenu" href="kit4-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit4-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit4-help.php">Help center</a></li>
                        <li><a class="is-submenu" href="kit4-help-category.php">Help category</a></li>
                        <li><a class="is-submenu" href="kit4-help-article.php">Help article</a></li>
                        <li><a class="is-submenu" href="kit4-signup.php">Sign Up</a></li>
                        <li><a class="is-submenu" href="kit4-login.php">Login</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">toys</span>Landing kit 5</a>
                    <ul>
                        <li><a class="is-submenu" href="kit5-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit5-features.php">Feature page</a></li>
                        <li><a class="is-submenu" href="kit5-pricing.php">Pricing Page</a></li>
                        <li><a class="is-submenu" href="kit5-login.php">Login / Signup</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">mouse</span>Landing kit 6</a>
                    <ul>
                        <li><a class="is-submenu" href="kit6-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit6-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit6-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">work</span>Landing kit
                        7</a>
                    <ul>
                        <li><a class="is-submenu" href="kit7-landing.php">Landing page</a></li>
                        <li><a class="is-submenu" href="kit7-landing.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit7-authentication.php">Auth page</a></li>
                        <li><a class="is-submenu" href="kit7-pricing.php">Pricing page</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-cards.php#">
                        <span class="material-icons">public</span>Landing kit 8</a>
                    <ul>
                        <li><a class="is-submenu" href="kit8-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit8-landing-1.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit8-landing-2.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit8-landing-3.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit8-landing-4.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit8-pricing.php">Pricing page</a></li>
                        <li><a class="is-submenu" href="kit8-jobs.php">Jobs page</a></li>
                        <li><a class="is-submenu" href="kit8-login.php">Login page</a></li>
                        <li><a class="is-submenu" href="kit8-signup.php">Signup page</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-cards.php#">
                        <span class="material-icons">assessment</span>Landing kit 9</a>
                    <ul>
                        <li><a class="is-submenu" href="kit9-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit9-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit9-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit9-landing-4.php">Landing page v4</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-cards.php#">
                        <span class="material-icons">account_balance</span>Landing kit 10</a>
                    <ul>
                        <li><a class="is-submenu" href="kit10-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit10-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit10-landing-3.php">Landing page v3</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-cards.php#">
                        <span class="material-icons">shop</span>Landing kit 11</a>
                    <ul>
                        <li><a class="is-submenu" href="kit11-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit11-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit11-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit11-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit11-landing-5.php">Landing page v5</a></li>
                        <li><a class="is-submenu" href="kit11-landing-6.php">Landing page v6</a></li>
                        <li><a class="is-submenu" href="kit11-landing-7.php">Landing page v7</a></li>
                        <li><a class="is-submenu" href="kit11-landing-8.php">Landing page v8</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-cards.php#">
                        <span class="material-icons">stars</span>Landing kit 12</a>
                    <ul>
                        <li><a class="is-submenu" href="kit12-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit12-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit12-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit12-landing-4.php">Landing page v4</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-cards.php#">
                        <span class="material-icons">new_releases</span>Landing kit 13</a>
                    <ul>
                        <li><a class="is-submenu" href="kit13-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit13-landing-2.php">Landing page v2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-cards.php#">
                        <span class="material-icons">code</span>Landing kit 14</a>
                    <ul>
                        <li><a class="is-submenu" href="kit14-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit14-landing-2.php">Landing page v2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link" href="_components-basicui-cards.php#">
                        <span class="material-icons">settings</span>Landing kit 15</a>
                    <ul>
                        <li><a class="is-submenu" href="kit15-landing.php">Landing page v1</a></li>
                        <li><a class="is-submenu" href="kit15-landing-2.php">Landing page v2</a></li>
                        <li><a class="is-submenu" href="kit15-landing-3.php">Landing page v3</a></li>
                        <li><a class="is-submenu" href="kit15-landing-4.php">Landing page v4</a></li>
                        <li><a class="is-submenu" href="kit15-about.php">About page</a></li>
                        <li><a class="is-submenu" href="kit15-contact.php">Contact page</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- Navigation menu -->
        <div id="onepagers" class="navigation-menu-wrapper animated preFadeInRight fadeInRight is-hidden">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>Sub Pages</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-cards.php#"><span class="material-icons">domain</span>Company</a>
                    <ul>
                        <li><a class="is-submenu" href="about-page-1.php">About v1</a></li>
                        <li><a class="is-submenu" href="about-page-2.php">About v2</a></li>
                        <li><a class="is-submenu" href="case-study-1.php">Case Study v1</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-cards.php#"><span class="material-icons">call</span>Contact</a>
                    <ul>
                        <li><a class="is-submenu" href="contact-page-1.php">Contact v1</a></li>
                        <li><a class="is-submenu" href="contact-page-2.php">Contact v2</a></li>
                        <li><a class="is-submenu" href="contact-page-3.php">Contact v3</a></li>
                        <li><a class="is-submenu" href="contact-page-4.php">Contact v4</a></li>
                        <li><a class="is-submenu" href="contact-page-5.php">Contact v5</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link has-new" href="_components-basicui-cards.php#"><span class="material-icons">book</span>Blog</a>
                    <ul>
                        <li><a class="is-submenu" href="blog-posts-full.php">Posts List v1</a></li>
                        <li><a class="is-submenu" href="blog-posts-full-alt.php">Posts List v2</a></li>
                        <li><a class="is-submenu" href="blog-posts-side.php">Posts List v3</a></li>
                        <li><a class="is-submenu" href="blog-posts-side-alt.php">Posts List v4</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-full.php">Posts Grid v1</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-full-masonry.php">Posts Grid v2</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-side.php">Posts Grid v3</a></li>
                        <li><a class="is-submenu" href="blog-posts-grid-side-masonry.php">Posts Grid v4</a></li>
                        <li><a class="is-submenu" href="blog-single-full.php">Single Post V1</a></li>
                        <li><a class="is-submenu" href="blog-single-side.php">Single Post V2</a></li>
                    </ul>
                </li>
                <li class="has-children">
                    <a class="parent-link has-new" href="_components-basicui-cards.php#"><span class="material-icons">highlight</span>Error Pages</a>
                    <ul>
                        <li><a class="is-submenu" href="error-1.php">Error v1</a></li>
                        <li><a class="is-submenu" href="error-2.php">Error v2</a></li>
                        <li><a class="is-submenu" href="error-3.php">Error v3</a></li>
                        <li><a class="is-submenu" href="error-4.php">Error v4</a></li>
                        <li><a class="is-submenu" href="error-5.php">Error v5</a></li>
                        <li><a class="is-submenu" href="error-6.php">Error v6</a></li>
                        <li><a class="is-submenu" href="error-7.php">Error v7</a></li>
                        <li><a class="is-submenu" href="error-8.php">Error v8</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- Navigation menu -->
        <div id="components" class="navigation-menu-wrapper animated preFadeInRight fadeInRight is-hidden">
            <!-- Navigation Header -->
            <div class="navigation-menu-header">
                <span>Components</span>
                <a class="ml-auto hamburger-btn navigation-close" href="javascript:void(0);">
                    <span class="menu-toggle">
                            <span class="icon-box-toggle">
                                <span class="rotate">
                                    <i class="icon-line-top"></i>
                                    <i class="icon-line-center"></i>
                                    <i class="icon-line-bottom"></i>
                                </span>
                    </span>
                    </span>
                </a>
            </div>
            <!-- Navigation body -->
            <ul class="navigation-menu">
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">view_quilt</span>Layout</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-layout-grid.php">Grid system</a></li>
                        <li><a class="is-submenu" href="_components-layout-video.php">Video background</a></li>
                        <li><a class="is-submenu" href="_components-layout-parallax.php">Parallax</a></li>
                        <li><a class="is-submenu" href="_components-layout-headers.php">Headers</a></li>
                        <li><a class="is-submenu" href="_components-layout-footers.php">Footers</a></li>
                        <li><a class="is-submenu" href="_components-layout-typography.php">Typography</a></li>
                        <li><a class="is-submenu" href="_components-layout-colors.php">Colors</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">subject</span>Navbars</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-layout-navbar-fade-light.php">Fade light</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-fade-dark.php">Fade dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-light.php">Static
                                light</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-dark.php">Static
                                dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-static-solid.php">Static
                                solid</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-double-dark.php">Double
                                dark</a></li>
                        <li><a class="is-submenu" href="_components-layout-navbar-double-light.php">Double
                                light</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">view_stream</span>Sections</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-sections-features.php">Features</a></li>
                        <li><a class="is-submenu" href="_components-sections-pricing.php">Pricing</a></li>
                        <li><a class="is-submenu" href="_components-sections-team.php">Team</a></li>
                        <li><a class="is-submenu" href="_components-sections-testimonials.php">Testimonials</a></li>
                        <li><a class="is-submenu" href="_components-sections-clients.php">Clients</a></li>
                        <li><a class="is-submenu" href="_components-sections-counters.php">Counters</a></li>
                        <li><a class="is-submenu" href="_components-sections-carousel.php">Carousel</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">playlist_add_check</span>Basic
                        UI</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-basicui-cards.php">Cards</a></li>
                        <li><a class="is-submenu" href="_components-basicui-buttons.php">Buttons</a></li>
                        <li><a class="is-submenu" href="_components-basicui-dropdowns.php">Dropdowns</a></li>
                        <li><a class="is-submenu" href="_components-basicui-lists.php">Lists</a></li>
                        <li><a class="is-submenu" href="_components-basicui-modals.php">Modals</a></li>
                        <li><a class="is-submenu" href="_components-basicui-tabs.php">Tabs & pills</a></li>
                        <li><a class="is-submenu" href="_components-basicui-accordion.php">Accordions</a></li>
                        <li><a class="is-submenu" href="_components-basicui-badges.php">Badges & labels</a></li>
                        <li><a class="is-submenu" href="_components-basicui-popups.php">Popups</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">playlist_add</span>Advanced
                        UI</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-advancedui-tables.php">Tables</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-timeline.php">Timeline</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-boxes.php">Boxes</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-messages.php">Messages</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-megamenu.php">Megamenu</a></li>
                        <li><a class="is-submenu" href="_components-advancedui-calendar.php">Calendar</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">check_box</span>Forms</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-forms-inputs.php">Inputs</a></li>
                        <li><a class="is-submenu" href="_components-forms-controls.php">Controls</a></li>
                        <li><a class="is-submenu" href="_components-forms-layouts.php">Form layouts</a></li>
                        <li><a class="is-submenu" href="_components-forms-steps.php">Step forms</a></li>
                        <li><a class="is-submenu" href="_components-forms-uploader.php">Uploader</a></li>
                    </ul>
                </li>
                <li class="has-children"><a class="parent-link" href="_components-basicui-cards.php#"><span class="material-icons">adjust</span>Icons</a>
                    <ul>
                        <li><a class="is-submenu" href="_components-icons-im.php">Icons Mind</a></li>
                        <li><a class="is-submenu" href="_components-icons-sl.php">Simple Line Icons</a></li>
                        <li><a class="is-submenu" href="_components-icons-fa.php">Font Awesome</a></li>
                        <li><a class="is-submenu" href="https://material.io/icons/" target="_blank">Material Icons</a></li>
                        <li><a class="is-submenu" href="_components-extensions-iconpicker.php">Icon Picker</a></li>
                    </ul>
                </li>
            </ul>
        </div>
        <!-- /Navigation menu -->
    </div>
    <!-- /Side navigation -->
    <!-- Back To Top Button -->
    <div id="backtotop"><a href="_components-basicui-cards.php#"></a></div> <!-- Concatenated jQuery and plugins -->
    <script src="assets/js/app.js"></script>

    <!-- Bulkit js -->
    <script src="assets/js/functions.js"></script>
    <script src="assets/js/main.js"></script>
</body>

</html>