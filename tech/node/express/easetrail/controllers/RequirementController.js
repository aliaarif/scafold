...
const list = async (req, res) => {
    try {
        ...
        const scategories = await SubcategoryController.list();
        if (categories.length > 0) {
            for (let i = 0; i < categories.length; i++) {
                const businesssData = [];
                const catId = categories[i]['_id'].toString();
                const catBusinesses = await Business.find({ category_id: catId });

                if (catBusinesses.length > 0) {
                    for (let j = 0; j < catBusinesses.length; j++) {
                        const scatId = scategories[j]['_id'].toString();
                        const scatBusinesses = await Business.find({ subcategory_id: scatId });
                        for (let k = 0; k < scatBusinesses.length; k++) {
                            const sCat = await Business.find({ _id: scatId });
                            businesssData.push({
                                "subcategory": sCat['name'],
                                "name": catBusinesses[k]['name'],
                                "slug": catBusinesses[k]['slug'],
                                "images": catBusinesses[k]['images'],
                            });
                        }
                    }
                }
